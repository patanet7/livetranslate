...
class ErrorHandlingValidator:
    ...
    def __init__(self, orchestration_url="http://localhost:3000", whisper_url="http://localhost:5001"):
    ...
    def create_test_audio(self, duration: float = 1.0, sample_rate: int = 16000, format: str = "wav") -> bytes:
    ...
    def test_audio_format_error(self):
        ...
        try:
            ...
            if is_error_handled and response.headers.get('content-type', '').startswith('application/json'):
    ...
    def test_audio_corruption_error(self):
        ...
        try:
            ...
            if is_error_handled:
                ...
                self.log_test_result("AudioCorruptionError", False, {"status_code": response.status_code})
        ...
        try:
            ...
            if is_validation_error:
                try:
                    ...
                    self.log_test_result("ValidationError", is_validation_error, {"status_code": response.status_code})
            else:
                self.log_test_result("ValidationError", False, {"status_code": response.status_code})
    ...
    def test_file_size_validation(self):
        ...
        try:
            ...
            is_size_error = response.status_code in [413, 422]  # Payload too large or validation error
            ...
            if is_size_error:
                try:
                    ...
                    self.log_test_result("FileSizeValidation", is_size_error, {"status_code": response.status_code})
            else:
                self.log_test_result("FileSizeValidation", False, {"status_code": response.status_code})
    ...
    def test_valid_audio_processing(self):
        ...
        try:
            ...
            response = requests.post(f"{self.orchestration_url}/api/audio/upload", files=files, data=data)
            ...
            if response.status_code == 200:
                ...
                self.log_test_result("ValidAudioProcessing", False, {"status_code": response.status_code})
    ...
    def test_error_response_format(self):
        ...
        try:
            ...
            if response.status_code >= 400:
                try:
                    error_data = response.json()
                    has_error_structure = any(key in error_data for key in ['error', 'detail', 'message'])
    ...
    def generate_report(self):
        ...
        logger.info("\n" + "="*60)
        logger.info(f"📊 ERROR HANDLING VALIDATION REPORT")
        logger.info("="*60)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--orchestration-url', default='http://localhost:3000', 
                       help='Orchestration service URL')
    parser.add_argument('--whisper-url', default='http://localhost:5001', 
                       help='Whisper service URL')
