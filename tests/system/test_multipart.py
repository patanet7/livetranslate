...
def test_direct_whisper():
    ...
    try:
        response = requests.post(
            "http://localhost:5001/transcribe/whisper-base", files=files, timeout=10
        )
...
def test_gateway_forwarding():
    ...
    try:
        response = requests.post(
            "http://localhost:3000/api/whisper/transcribe/whisper-base", files=files, timeout=10
        )
...
def test_debug_endpoint():
    ...
    try:
        response = requests.post(
            "http://localhost:3000/api/debug/multipart", files=files, timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            for file_info in data.get("files_received", []):
                print(
                    f"  File: {file_info.get('filename')} ({file_info.get('size')} bytes, {file_info.get('content_type')})"
                )
        else:
