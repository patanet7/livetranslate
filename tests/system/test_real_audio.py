...
def test_direct_whisper():
    ...
    try:
        response = requests.post('http://localhost:5001/transcribe/whisper-base', files=files, timeout=30)
...
def test_gateway_forwarding():
    ...
    try:
        response = requests.post('http://localhost:3000/api/whisper/transcribe/whisper-base', files=files, timeout=30)
...
def test_orchestration_audio_endpoint():
    ...
    try:
        ...
        response = requests.post(
            'http://localhost:3000/api/whisper/transcribe/whisper-base', 
            files=files,
            timeout=30
        )
