...
async def check_services() -> Dict[str, bool]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{ORCHESTRATION_URL}/api/health", timeout=aiohttp.ClientTimeout(total=5)) as resp:
        ...
        try:
            async with session.get(f"{TRANSLATION_URL}/health", timeout=aiohttp.ClientTimeout(total=5)) as resp:
...
async def translate_text(session: aiohttp.ClientSession, text: str, glossary: Dict[str, str]) -> Dict[str, Any]:
...
async def run_e2e_test(results: TestResults, speed_multiplier: float = 10.0):
    ...
    logger.info(f"Running {total_entries} entries over {test_duration_sec:.1f}s ({speed_multiplier}x speed)")
    logger.info(f"Watch captions at: {ORCHESTRATION_URL}/static/captions.html?session={TEST_SESSION_ID}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        results.add_pass(
            ...
            f"{translation_count}/{total_entries} translated successfully"
        )
        results.add_pass(
            "Caption Broadcast",
            f"{caption_count}/{total_entries} captions sent to WebSocket"
        )
