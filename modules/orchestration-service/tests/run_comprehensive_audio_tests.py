...
class TestSuiteRunner:
    ...
    def generate_report(self, output_file: str = "comprehensive_test_report.json"):
        ...
        passed_categories = [cat for cat, result in self.results.items() if result.get("exit_code") == 0]
        failed_categories = [cat for cat, result in self.results.items() if result.get("exit_code") != 0]
...
def main():
    ...
    parser.add_argument(
        "--categories", 
        nargs="+", 
        choices=["unit", "integration", "performance", "error_handling", "format_compatibility"],
        default=None,
