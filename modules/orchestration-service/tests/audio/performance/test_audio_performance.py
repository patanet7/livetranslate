...
class PerformanceMonitor:
    ...
    def get_metrics(self, operations_count: int = 1, audio_duration: float = 0.0) -> PerformanceMetrics:
...
class TestAudioProcessorPerformance:
    ...
    @pytest.mark.asyncio
    async def test_single_stage_processing_performance(self, sample_audio_data, performance_test_config):
        ...
        for stage_config in performance_test_config["processing_stages"]:
            ...
            assert metrics.latency_ms < performance_test_config["max_processing_time_ms"][stage_config[0]]
            assert metrics.success_rate > 0.95
        
        # Verify performance scaling with pipeline complexity
        single_stage_latency = stage_results["['vad']"].latency_ms
        full_pipeline_latency = stage_results[str(performance_test_config["processing_stages"][-1])].latency_ms
        
        # Full pipeline should be slower but not linearly
        assert full_pipeline_latency > single_stage_latency
        assert full_pipeline_latency < single_stage_latency * len(performance_test_config["processing_stages"][-1])
    ...
    @pytest.mark.asyncio
    async def test_concurrent_processing_performance(self, sample_audio_data, performance_test_config):
        ...
        for num_concurrent in performance_test_config["concurrent_sessions"]:
            ...
            assert metrics.memory_usage_mb < performance_test_config["max_memory_usage_mb"] * num_concurrent
...
class TestChunkManagerPerformance:
    ...
    @pytest.mark.asyncio
    async def test_chunking_throughput_performance(self, mock_database_adapter, sample_audio_data, temp_dir):
        ...
        for chunk_duration in chunk_sizes:
            ...
            manager = create_chunk_manager(config, mock_database_adapter, session_id, SourceType.BOT_AUDIO)
            await manager.start()
...
class TestAudioCoordinatorPerformance:
    ...
    @pytest.mark.asyncio
    async def test_session_management_performance(self, mock_database_adapter, mock_service_urls, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_audio_processing_performance(self, audio_coordinator, sample_audio_data):
    ...
    async def test_service_communication_performance(self, audio_coordinator, sample_audio_data):
        ...
        async def mock_service_call(*args, **kwargs):
            ...
            return type('Response', (), {
                'status_code': 200,
                'json': lambda: {"text": "Fast response", "speaker_id": "speaker_0"}
            })()
    ...
    async def test_memory_stress_test(self, audio_coordinator, test_audio_fixtures):
        ...
        with patch.object(audio_coordinator, '_process_with_services') as mock_process:
            ...
            for i in range(0, len(large_audio), chunk_size):
                ...
                try:
                    result = await audio_coordinator.add_audio_data(session_id, audio_chunk)
