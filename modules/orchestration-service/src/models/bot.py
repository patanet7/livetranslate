...
class AudioCaptureStats(BaseModel):
    ...
    average_quality_score: float = Field(
        description="Average audio quality score (0-1)", ge=0.0, le=1.0, json_schema_extra={"example": 0.87}
    )
    last_capture_timestamp: datetime = Field(description="Last capture timestamp")
    device_info: str = Field(
        description="Audio device information", json_schema_extra={"example": "Default Audio Device"}
    )
    sample_rate_actual: int = Field(
        description="Actual sample rate being used", json_schema_extra={"example": 16000}
    )
    channels_actual: int = Field(description="Actual number of channels", json_schema_extra={"example": 1})
...
class CaptionProcessorStats(BaseModel):
    ...
    average_confidence: float = Field(
        description="Average caption confidence (0-1)", ge=0.0, le=1.0, json_schema_extra={"example": 0.92}
    )
    last_caption_timestamp: datetime = Field(
        description="Last caption processed timestamp"
    )
    processing_latency_ms: float = Field(
        description="Average processing latency in milliseconds", json_schema_extra={"example": 125.3}
    )
...
class VirtualWebcamStats(BaseModel):
    ...
    frames_generated: int = Field(description="Total frames generated", json_schema_extra={"example": 54000})
    ...
    average_fps: float = Field(description="Average frames per second", json_schema_extra={"example": 29.7})
...
class TimeCorrelationStats(BaseModel):
    ...
    success_rate: float = Field(
        description="Correlation success rate (0-1)", ge=0.0, le=1.0, json_schema_extra={"example": 0.94}
    )
    ...
    correlation_accuracy: float = Field(
        description="Correlation accuracy score (0-1)", ge=0.0, le=1.0, json_schema_extra={"example": 0.88}
    )
...
class BotPerformanceStats(BaseModel):
    ...
    cpu_usage_percent: float = Field(
        description="Average CPU usage percentage", ge=0.0, le=100.0, json_schema_extra={"example": 25.8}
    )
    memory_usage_mb: float = Field(description="Memory usage in MB", json_schema_extra={"example": 512.7})
    network_bytes_sent: int = Field(description="Network bytes sent", json_schema_extra={"example": 1048576})
    ...
    error_count: int = Field(description="Number of errors encountered", json_schema_extra={"example": 2})
...
class BotInstance(IDMixin, TimestampMixin):
    ...
    bot_id: str = Field(description="Bot identifier", json_schema_extra={"example": "bot_abc123def456"})
...
class BotResponse(ResponseMixin, TimestampMixin):
    ...
    operation: str = Field(description="Operation performed", json_schema_extra={"example": "spawn"})
...
class BotStats(BaseModel):
    ...
    total_bots_spawned: int = Field(description="Total bots spawned", json_schema_extra={"example": 125})
    active_bots: int = Field(description="Currently active bots", json_schema_extra={"example": 8})
    completed_sessions: int = Field(description="Completed bot sessions", json_schema_extra={"example": 115})
    failed_sessions: int = Field(description="Failed bot sessions", json_schema_extra={"example": 2})
