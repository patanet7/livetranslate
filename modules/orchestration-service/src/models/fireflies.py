...
class FirefliesChunk(BaseModel):
    ...
    transcript_id: str = Field(
        description="Unique identifier for the transcript/meeting"
    )
    chunk_id: str = Field(
...
class FirefliesMeeting(BaseModel):
    ...
    organizer_email: Optional[str] = Field(
        default=None, description="Organizer email address"
    )
    meeting_link: Optional[str] = Field(
        default=None, description="Original meeting link"
    )
    start_time: Optional[datetime] = Field(
        default=None, description="Meeting start time"
    )
    end_time: Optional[datetime] = Field(default=None, description="Meeting end time")
    privacy: Optional[str] = Field(default=None, description="Privacy setting")
...
class FirefliesSessionConfig(BaseModel):
    ...
    transcript_id: str = Field(
        description="Transcript ID to connect to (from active_meetings)"
    )
    ...
    glossary_id: Optional[str] = Field(
        default=None, description="Glossary ID to use for this session"
    )
...
class FirefliesSession(TimestampMixin):
    ...
    fireflies_meeting_id: Optional[str] = Field(
        default=None, description="Fireflies meeting ID"
    )
    ...
    last_chunk_time: Optional[datetime] = Field(
        default=None, description="Last chunk received timestamp"
    )
    last_chunk_id: Optional[str] = Field(
        default=None, description="Last processed chunk ID"
    )

    # Configuration
    config: Optional[FirefliesSessionConfig] = Field(
        default=None, description="Session configuration"
    )

    # Statistics
    chunks_received: int = Field(default=0, description="Total chunks received")
    sentences_produced: int = Field(
        default=0, description="Complete sentences produced"
    )
...
class SpeakerBuffer(BaseModel):
    ...
    chunks: List[FirefliesChunk] = Field(
        default_factory=list, description="Accumulated chunks"
    )
    buffer_start_time: Optional[float] = Field(
    ...
    def reset_to(
        ...
        saved_transcript_id = (
            self.chunks[0].transcript_id if self.chunks else transcript_id
        )
        self.chunks = []
        if text.strip():
    ...
    @property
    def last_chunk(self) -> Optional[FirefliesChunk]:
...
class TranslationResult(TimestampMixin):
    ...
    context_sentences_used: int = Field(
        default=0, description="Number of context sentences used"
    )
    ...
    translation_unit_id: Optional[str] = Field(
...
class CaptionEntry(BaseModel):
    ...
    original_text: Optional[str] = Field(
        default=None, description="Original text (if showing both)"
    )
    ...
    speaker_color: Optional[str] = Field(
        default=None, description="Color for speaker (hex)"
    )
    target_language: str = Field(description="Target language code")
    timestamp: datetime = Field(
...
class FirefliesConnectRequest(BaseModel):
    ...
    glossary_id: Optional[str] = Field(default=None, description="Glossary to use")
    domain: Optional[str] = Field(
...
class FirefliesConnectResponse(ResponseMixin):
    ...
    meeting_info: Optional[FirefliesMeeting] = Field(
...
class FirefliesStatusResponse(ResponseMixin):
    ...
    session: Optional[FirefliesSession] = Field(
        default=None, description="Session details"
    )
