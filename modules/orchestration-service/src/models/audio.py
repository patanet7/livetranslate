...
class AudioConfiguration(BaseModel):
    ...
    compressor_ratio: float = Field(
        default=4.0, description="Compressor ratio", ge=1.0, le=20.0
    )
...
class AudioProcessingRequest(BaseModel):
    ...
    file_upload: Optional[str] = Field(
        default=None, description="File upload reference"
    )
...
class AudioProcessingResult(BaseModel):
    ...
    input_level_db: Optional[float] = Field(
        default=None, description="Input audio level in dB"
    )
    output_level_db: Optional[float] = Field(
        default=None, description="Output audio level in dB"
    )
    quality_score: Optional[float] = Field(
        default=None, description="Quality score (0-1)", ge=0.0, le=1.0
    )
    artifacts_detected: bool = Field(
        default=False, description="Whether artifacts were detected"
    )
...
class AudioProcessingResponse(ResponseMixin, TimestampMixin):
    ...
    # Output
    output_audio_url: Optional[str] = Field(
        default=None, description="URL to processed audio file"
    )
    output_audio_data: Optional[str] = Field(
    ...
    stream_chunk_id: Optional[int] = Field(
