    ...
    def _load_env_overrides(self):
        ...
        for env_var, config_path in env_mappings.items():
            ...
            if value is not None:
                ...
                logger.debug(
                    f"Environment override: {env_var} -> {'.'.join(config_path)}"
                )
    ...
    def _validate_config(self):
        ...
        timeouts = [
            (
                ...
                self.config.get("websocket", {}).get("connection_timeout", 1800),
            ),
            ("gateway.timeout", self.config.get("gateway", {}).get("timeout", 30)),
            (
                "monitoring.health_check_interval",
                self.config.get("monitoring", {}).get("health_check_interval", 10),
            ),
        ]
