...
class WhisperIntegration:
    ...
    def add_routes(self, app: Flask, socketio: SocketIO):
        ...
        def list_whisper_models():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "models", request
                )
        ...
        def whisper_health():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "health", request
                )
            except Exception as e:
        ...
        def whisper_status():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "status", request
                )
            except Exception as e:
        ...
        def create_whisper_session():
            ...
            try:
                ...
                response = self.api_gateway.route_request(
                    self.service_name, "sessions", request
                )
        ...
        def get_whisper_session(session_id: str):
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, f"sessions/{session_id}", request
                )
            except Exception as e:
        ...
        def configure_whisper_streaming():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "stream/configure", request
                )
            except Exception as e:
        ...
        def start_whisper_streaming():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "stream/start", request
                )
            except Exception as e:
        ...
        def stop_whisper_streaming():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "stream/stop", request
                )
            except Exception as e:
        ...
        def stream_whisper_audio():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "stream/audio", request
                )
            except Exception as e:
        ...
        def get_whisper_transcriptions():
            ...
            try:
                return self.api_gateway.route_request(
                    self.service_name, "stream/transcriptions", request
                )
            except Exception as e:
        ...
        def handle_whisper_stream(data):
            ...
            try:
                ...
                if not session_id or not audio_data:
                    emit(
                        "whisper_error", {"error": "session_id and audio_data required"}
                    )
                    return
    ...
    def _handle_transcription_request(
        ...
        try:
            ...
            response = self.api_gateway.route_request(
                self.service_name, api_path, request
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.session_lock:
            ...
            websocket_count = len(
                [s for s in self.active_sessions.values() if "websocket_sid" in s]
            )
...
def add_whisper_routes(
    app: Flask, socketio: SocketIO, api_gateway, websocket_manager
) -> WhisperIntegration:
