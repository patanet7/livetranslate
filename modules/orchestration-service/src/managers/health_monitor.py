...
class HealthMonitor:
    ...
    async def _check_service_health(self, service_name: str):
        ...
        try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(health_url) as response:
                        ...
                        if response.status == 200:
                            ...
                            service.last_error = None
                            logger.debug(
                                f"Service {service_name} health check successful: {health_url}"
                            )
                        else:
                            ...
                            logger.warning(
                                f"Service {service_name} health check failed with HTTP {response.status}: {health_url}"
                            )
            ...
            if (
                "connect call failed" in error_msg.lower()
            ...
            # Check if this is an SSL error on an HTTP endpoint
                ...
                logger.error(
                    f"SSL error for {service_name} at {health_url} - this usually means HTTPS is being forced on an HTTP endpoint"
                )
    ...
    async def get_system_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "monitoring": {"health_check_interval": 30, "timeout": 5},
            "timestamp": time.time(),
        }
            ...
            return {
                ...
                "services_detail": {
                    name: {
                        ...
                        "uptime_seconds": time.time() - service.last_check
