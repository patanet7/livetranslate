...
class UnifiedConfigurationManager:
    ...
    async def sync_configuration(self, target_services: Optional[List[str]] = None) -> bool:
            ...
            for name, manager in managers:
                try:
                    if hasattr(manager, 'health_check'):
                        ...
                        manager_health = {"status": "healthy", "note": "no health check method"}
    ...
    async def sync_all_configurations(self) -> bool:
        ...
        async with self._sync_lock:
            try:
                ...
                core_sync = self.core_config.reload() if hasattr(self.core_config, 'reload') else True
                audio_sync = self.audio_config.reload() if hasattr(self.audio_config, 'reload') else True
                ...
                if success:
                    logger.info("Unified configuration synchronization completed successfully")
                else:
                    logger.warning("Unified configuration synchronization completed with issues")
