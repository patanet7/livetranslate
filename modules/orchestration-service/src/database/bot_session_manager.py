...
class AudioFileManager:
    ...
    async def store_audio_file(self, session_id: str, audio_data: bytes, 
                              file_format: str = 'wav', metadata: Dict = None) -> str:
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                ...
                """, file_id, session_id, str(file_path), file_name, len(audio_data),
                file_format, duration, sample_rate, channels, chunk_start, chunk_end,
                quality_score, 'completed', file_hash, json.dumps(metadata or {}),
...
class TranscriptManager:
    ...
    async def store_transcript(self, session_id: str, source_type: str, 
                              transcript_text: str, language_code: str,
                              start_timestamp: float, end_timestamp: float,
                              speaker_info: Dict = None, audio_file_id: str = None,
        ...
        try:
            ...
            confidence_score = processing_metadata.get('confidence_score') if processing_metadata else None
            segment_index = processing_metadata.get('segment_index', 0) if processing_metadata else 0
            google_entry_id = processing_metadata.get('google_transcript_entry_id') if processing_metadata else None
            ...
            async with self.db_pool.acquire() as conn:
                ...
                language_code, start_timestamp, end_timestamp, speaker_id,
                speaker_name, confidence_score, segment_index, audio_file_id,
                google_entry_id, json.dumps(processing_metadata or {}),
                datetime.now(), datetime.now())
    ...
    async def get_session_transcripts(self, session_id: str, 
                                    source_type: str = None) -> List[TranscriptRecord]:
    ...
    async def get_transcript_by_timerange(self, session_id: str, 
                                        start_time: float, end_time: float) -> List[TranscriptRecord]:
...
class TranslationManager:
    ...
    async def store_translation(self, session_id: str, source_transcript_id: str,
                               translated_text: str, source_language: str,
                               target_language: str, translation_service: str,
                               speaker_info: Dict = None, timing_info: Dict = None,
                               processing_metadata: Dict = None) -> str:
        ...
        try:
            ...
            speaker_id = speaker_info.get('speaker_id') if speaker_info else None
            speaker_name = speaker_info.get('speaker_name') if speaker_info else None
            confidence = processing_metadata.get('translation_confidence') if processing_metadata else None
            ...
            async with self.db_pool.acquire() as conn:
                ...
                """, translation_id, session_id, source_transcript_id, translated_text,
                source_language, target_language, confidence, translation_service,
                speaker_id, speaker_name, start_timestamp, end_timestamp,
                json.dumps(processing_metadata or {}), datetime.now(), datetime.now())
            
            logger.debug(f"Stored translation: {translation_id} ({source_language} -> {target_language})")
    ...
    async def get_session_translations(self, session_id: str, 
                                     target_language: str = None) -> List[TranslationRecord]:
...
class CorrelationManager:
    ...
    async def store_correlation(self, session_id: str, 
                               ...
                               correlation_metadata: Dict = None) -> str:
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                ...
                """, correlation_id, session_id, google_transcript_id, inhouse_transcript_id,
                correlation_confidence, timing_offset, correlation_type, correlation_method,
                speaker_id, start_timestamp, end_timestamp,
                json.dumps(correlation_metadata or {}), datetime.now())
            ...
            async with self.db_pool.acquire() as conn:
                ...
                return [
                    CorrelationRecord(
                        ...
                        start_timestamp=row['start_timestamp'],
                        end_timestamp=row['end_timestamp'],
                        correlation_metadata=json.loads(row['correlation_metadata']) if row['correlation_metadata'] else {},
                        created_at=row['created_at']
                    )
                    for row in rows
                ]
...
class BotSessionDatabaseManager:
    ...
    async def get_comprehensive_session_data(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'translations': {
                    'by_language': {k: [asdict(t) for t in v] for k, v in translations_by_language.items()},
                ...
                'statistics': {
                    ...
                    'languages_detected': list(set(t.language_code for t in transcripts)),
    ...
    async def cleanup_session(self, session_id: str, remove_files: bool = False) -> bool:
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                await conn.execute("DELETE FROM bot_sessions.sessions WHERE session_id = $1", session_id)
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if not schema_exists:
                    logger.warning("bot_sessions schema does not exist - some features may not work")
                else:
    ...
    async def _update_statistics(self):
        ...
        try:
            async with self.db_pool.acquire() as conn:
                self.total_sessions = await conn.fetchval("SELECT COUNT(*) FROM bot_sessions.sessions") or 0
                self.total_audio_files = await conn.fetchval("SELECT COUNT(*) FROM bot_sessions.audio_files") or 0
                self.total_transcripts = await conn.fetchval("SELECT COUNT(*) FROM bot_sessions.transcripts") or 0
                self.total_translations = await conn.fetchval("SELECT COUNT(*) FROM bot_sessions.translations") or 0
                self.total_correlations = await conn.fetchval("SELECT COUNT(*) FROM bot_sessions.correlations") or 0
    ...
    async def get_database_statistics(self) -> Dict[str, Any]:
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                # Storage usage
                total_audio_size = await conn.fetchval("""
...
# Factory function
def create_bot_session_manager(database_config: Dict[str, Any], 
                              audio_storage_path: str) -> BotSessionDatabaseManager:
...
async def main():
    ...
    transcript_id = await manager.transcript_manager.store_transcript(
        session_id, 'whisper_service', 'Hello everyone welcome to the meeting',
        'en', 0.0, 3.5, {'speaker_id': 'speaker_1', 'speaker_name': 'John Doe'},
        file_id, {'confidence_score': 0.95}
    )
    print(f"Stored transcript: {transcript_id}")
    
    # Store translation
    translation_id = await manager.translation_manager.store_translation(
        session_id, transcript_id, 'Hola a todos, bienvenidos a la reunión',
        'en', 'es', 'translation_service',
