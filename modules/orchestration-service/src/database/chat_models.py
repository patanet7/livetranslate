...
class User(Base):
    ...
    # Primary key
    user_id = Column(String(255), primary_key=True, index=True)  # External user ID (e.g., from OAuth)
...
class ConversationSession(Base):
    ...
    # Session metadata
    session_type = Column(String(50), nullable=False, default='user_chat')  # user_chat, bot_meeting, etc.
    ...
    __table_args__ = (
        ...
        Index('ix_conv_sessions_user_started', 'user_id', 'started_at'),  # Composite for user queries
...
class ChatMessage(Base):
    ...
    __table_args__ = (
        ...
        Index('ix_chat_messages_sequence', 'session_id', 'sequence_number'),  # Composite for ordering
        Index('ix_chat_messages_content_fulltext', 'content', postgresql_using='gin', postgresql_ops={'content': 'gin_trgm_ops'}),  # Full-text search
        Index('ix_chat_messages_translated_gin', 'translated_content', postgresql_using='gin'),
    )
