...
async def _perform_lufs_analysis(
    ...
    try:
        ...
        return {
            ...
            "recommendations": _generate_loudness_recommendations(measured_lufs, target_lufs, dynamic_range),
...
def _generate_loudness_recommendations(measured_lufs: float, target_lufs: float, dynamic_range: float) -> List[str]:
    ...
    if abs(lufs_diff) > 3.0:
        if lufs_diff > 0:
            recommendations.append(f"Reduce gain by {abs(lufs_diff):.1f} dB to meet target loudness")
        else:
            recommendations.append(f"Increase gain by {abs(lufs_diff):.1f} dB to meet target loudness")
    ...
    if dynamic_range < 6.0:
        recommendations.append("Audio is heavily compressed - consider using less aggressive compression")
