...
from src.dependencies import get_audio_coordinator, get_config_manager
...
async def _get_available_stages() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "limiter": {
            ...
            "default_config": {"threshold": -0.1, "release": 0.05, "lookahead": 0.005},
            "processing_time": "fast",
        },
        "voice_enhancement": {
    ...
    try:
        ...
        if stage_name == "vad":
            ...
            result = await _process_generic_stage(
                stage_name, audio_bytes, stage_config, correlation_id
            )
...
def _get_processing_time(speed: str) -> float:
    ...
    speed_map = {"fast": 0.02, "medium": 0.05, "slow": 0.1}
...
async def create_custom_pipeline(
    ...
    try:
        ...
        for i, stage_config in enumerate(pipeline_stages):
            ...
            logger.info(
                f"[{correlation_id}] Processing stage {i + 1}/{len(pipeline_stages)}: {stage_name}"
            )
