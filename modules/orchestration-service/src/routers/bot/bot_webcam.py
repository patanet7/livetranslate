    ...
    try:
        ...
        if hasattr(bot_instance, "virtual_webcam") and bot_instance.virtual_webcam:
            ...
            if frame_base64:
                ...
                raise get_error_response(
                    status.HTTP_404_NOT_FOUND, "No frame available", {"bot_id": bot_id}
                )
        else:
            raise get_error_response(
        ...
        if hasattr(bot_instance, "virtual_webcam") and bot_instance.virtual_webcam:
            ...
            if "max_translations_displayed" in config_update:
                config.max_translations_displayed = max(
                    1, min(10, int(config_update["max_translations_displayed"]))
                )

            if "translation_duration_seconds" in config_update:
                config.translation_duration_seconds = max(
                    5.0, min(60.0, float(config_update["translation_duration_seconds"]))
                )
