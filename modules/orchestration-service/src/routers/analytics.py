...
async def get_analytics_overview(
    ...
    try:
        ...
        overview = {
            ...
            "average_response_time": sum(s.response_time_ms for s in services_data)
            / len(services_data)
            if services_data
            else 0,
...
async def get_specific_metrics(
    ...
    try:
        ...
        if metric_type == MetricType.PERFORMANCE:
            ...
            metrics = await _get_usage_metrics(
                health_monitor, start_time, end_time, service, aggregation
            )
        elif metric_type == MetricType.QUALITY:
            metrics = await _get_quality_metrics(
                health_monitor, start_time, end_time, service, aggregation
            )
        elif metric_type == MetricType.ERRORS:
            metrics = await _get_error_metrics(
                health_monitor, start_time, end_time, service, aggregation
            )
        elif metric_type == MetricType.CAPACITY:
            metrics = await _get_capacity_metrics(
                health_monitor, start_time, end_time, service, aggregation
            )
        else:
...
async def get_websocket_analytics(
    ...
    try:
        ...
        if include_connection_details:
            analytics[
                "active_connections"
            ] = await websocket_manager.get_active_connection_details()
    ...
    try:
        ...
        for service in services_status:
            ...
            service_metric = ServiceMetrics(
                ...
                error_rate=service.get("error_count", 0)
                / max(1, overall_metrics.get("total_services", 1)),
                request_count=overall_metrics.get("total_services", 0),
                success_rate=1.0 if service.get("status") == "healthy" else 0.0,
                cpu_usage=overall_metrics.get("avg_response_time_ms", 0)
                / 10,  # Approximate CPU usage
...
async def _collect_audio_metrics(
    audio_client, start_time: datetime, end_time: datetime
...
async def _collect_translation_metrics(
...
async def _collect_websocket_metrics(
    websocket_manager, start_time: datetime, end_time: datetime
) -> WebSocketMetrics:
...
async def _collect_bot_metrics(
    bot_manager, start_time: datetime, end_time: datetime
) -> BotSessionMetrics:
