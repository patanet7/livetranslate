...
from .google_meet_client import GoogleMeetClient, BotManagerIntegration, create_google_meet_client

# Import database manager
from ..database.bot_session_manager import BotSessionDatabaseManager, create_bot_session_manager
...
class GoogleMeetBotManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.translation_service_url = self.config.get('translation_service_url', 'http://localhost:5003')
        ...
        logger.info(f"  Service URLs: whisper={self.whisper_service_url}, translation={self.translation_service_url}")
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if credentials_path:
                try:
                    ...
                    if success:
                        ...
                        logger.warning("Google Meet API initialization failed - continuing without API access")
                        self.google_meet_client = None
                        
                except Exception as e:
                    logger.warning(f"Google Meet API setup failed: {e} - continuing without API access")
                    self.google_meet_client = None
            else:
            ...
            try:
                ...
                audio_storage_path = self.config.get('audio_storage_path', '/tmp/livetranslate/audio')
                
                self.database_manager = create_bot_session_manager(database_config, audio_storage_path)
                success = await self.database_manager.initialize()
                
                if success:
                    logger.info("Bot session database manager initialized")
                else:
                    logger.warning("Database manager initialization failed - continuing without database features")
                    self.database_manager = None
            ...
            try:
                ...
                logger.warning(f"Lifecycle manager setup failed: {e} - continuing with basic lifecycle management")
                self.lifecycle_manager = None
            ...
            with self.lock:
                for bot in self.active_bots.values():
                    if bot.meeting_request.meeting_id == meeting_request.meeting_id:
                        logger.warning(f"Bot already exists for meeting: {meeting_request.meeting_id}")
                        return bot.bot_id
        ...
        try:
            ...
            with self.lock:
                if bot_id in self.active_bots:
                    ...
                    if self.database_manager:
                        try:
                            asyncio.create_task(self.database_manager.update_bot_session(bot_id, {
                                'status': 'ended',
                                'end_time': datetime.now(),
                                'performance_stats': bot.performance_stats
                            }))
                        except Exception as e:
                            logger.warning(f"Failed to update database session end for bot {bot_id}: {e}")
    ...
    async def _spawn_bot(self, bot: BotInstance):
        ...
        try:
            ...
            if self.database_manager:
                try:
                    await self.database_manager.update_bot_session(bot.bot_id, {
                        'status': 'active',
    ...
    async def _initialize_google_meet_integration(self, bot: BotInstance) -> bool:
        ...
        try:
            if not self.google_meet_client or not self.bot_manager_integration:
                logger.warning(f"Google Meet API not available - using fallback mode for bot {bot.bot_id}")
            ...
            meeting_uri = bot.meeting_request.metadata.get('meeting_uri') if bot.meeting_request.metadata else None
            
            if meeting_uri:
                # Join existing meeting
                meeting_info = await self.bot_manager_integration.join_external_meeting(meeting_uri)
                if meeting_info:
                    bot.google_meet_space_id = meeting_info.get('space', {}).get('name', f"external_{uuid.uuid4().hex[:8]}")
                ...
                meeting_result = await self.bot_manager_integration.create_bot_meeting(bot.meeting_request)
                if meeting_result:
                    ...
                    logger.info(f"Created new meeting for bot {bot.bot_id}: {meeting_info['meeting_uri']}")
                else:
    ...
    async def _start_bot_processes(self, bot: BotInstance) -> bool:
        ...
        try:
            ...
            if (self.google_meet_client and 
                self.bot_manager_integration and 
                bot.google_meet_space_id and 
                not bot.google_meet_space_id.startswith('mock_')):
                
                try:
                    success = await self.bot_manager_integration.monitor_bot_meeting(
                        bot.google_meet_space_id, 
                        bot.bot_id
                    )
                    if success:
                        ...
                        logger.warning(f"Failed to start Google Meet monitoring for bot {bot.bot_id}")
            ...
            if bot.performance_stats.get('start_time'):
                ...
                if total_meetings > 0:
                    self.average_meeting_duration = (
                        (self.average_meeting_duration * (total_meetings - 1) + duration) / total_meetings
                    )
            
            # Callback
            if self.on_meeting_ended:
                self.on_meeting_ended(
                    bot.meeting_request.meeting_id, 
                    bot.bot_id, 
                    bot.performance_stats
                )
    ...
    async def _process_bot_queue(self):
        ...
        try:
            with self.lock:
                while (self.bot_queue and 
                       len(self.active_bots) < self.config['max_concurrent_bots']):
        ...
        while self.running:
            try:
                ...
                for bot in bots_to_check:
                    ...
                    if health_score < 0.5:
                        logger.warning(f"Bot {bot.bot_id} health poor: {health_score:.2f}, issues: {issues}")
    ...
    def _check_bot_timeouts(self):
        ...
        timeout_duration = timedelta(seconds=self.config.get('meeting_timeout', 14400.0))
        ...
        try:
            ...
            if event_type == 'participant_update':
                ...
                bot.performance_stats['participants_seen'] = max(
                    bot.performance_stats.get('participants_seen', 0),
                    len(participants)
                )
    ...
    async def store_transcript(self, session_id: str, transcript_data: Dict) -> Optional[str]:
    ...
    async def store_translation(self, session_id: str, translation_data: Dict) -> Optional[str]:
    ...
    async def store_correlation(self, session_id: str, correlation_data: Dict) -> Optional[str]:
    ...
    def get_manager_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if self.google_meet_client:
                google_meet_status.update(self.google_meet_client.get_client_statistics())
