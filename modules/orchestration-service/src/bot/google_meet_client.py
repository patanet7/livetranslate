...
class GoogleMeetSpaceManager:
    ...
    async def create_space(self, config: Dict[str, Any] = None) -> Optional[MeetingSpace]:
...
class GoogleMeetConferenceManager:
    ...
    async def get_conference_record(self, conference_name: str) -> Optional[ConferenceRecord]:
    ...
    async def list_conference_records(self, filter_query: str = None) -> List[ConferenceRecord]:
    ...
    async def get_participants(self, conference_name: str) -> List[Participant]:
        ...
        try:
            request = self.service.conferenceRecords().participants().list(
            ...
            while request is not None:
                ...
                for participant in response.get('participants', []):
                    participants.append(Participant(
                        ...
                        display_name=participant.get('signedinUser', {}).get('displayName') or 
                                   participant.get('anonymousUser', {}).get('displayName')
                    ))
                
                request = self.service.conferenceRecords().participants().list_next(request, response)
...
class GoogleMeetTranscriptManager:
    ...
    async def list_transcripts(self, conference_name: str) -> List[str]:
        ...
        try:
            response = self.service.conferenceRecords().transcripts().list(
                parent=conference_name
            ).execute()
    ...
    async def get_transcript_entries(self, transcript_name: str) -> List[TranscriptEntry]:
        ...
        try:
            request = self.service.conferenceRecords().transcripts().entries().list(
            ...
            while request is not None:
                ...
                request = self.service.conferenceRecords().transcripts().entries().list_next(request, response)
...
class GoogleMeetClient:
    ...
    def is_ready(self) -> bool:
        ...
        return (self.authenticated and 
                self.authenticator.is_authenticated() and
                self.space_manager is not None)
    
    async def create_meeting_space(self, meeting_config: Dict[str, Any] = None) -> Optional[MeetingSpace]:
    ...
    async def monitor_active_conference(self, space_name: str, callback: callable = None) -> bool:
    ...
    async def _monitor_conference_loop(self, space_name: str):
        ...
        try:
            while space_name in self.monitored_spaces:
                ...
                participants = await self.conference_manager.get_participants(space.active_conference)
    ...
    def get_client_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'credentials_valid': self.authenticator.is_authenticated() if self.authenticator else False
        }
