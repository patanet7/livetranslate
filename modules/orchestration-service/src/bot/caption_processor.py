...
class GoogleMeetCaptionParser:
    ...
    def parse_caption(self, caption_text: str, timestamp: float) -> Tuple[Optional[CaptionSegment], Optional[SpeakerTimelineEvent]]:
        """
    ...
    def _parse_system_message(self, text: str, timestamp: float) -> Optional[SpeakerTimelineEvent]:
        ...
        for pattern in self.system_patterns:
            ...
            if match:
                if 'joined' in text:
                    ...
                    speaker_name = match.group(1).strip()
                    speaker_id = self._generate_speaker_id(speaker_name)
                    event_type = 'start_presenting' if 'is presenting' in text else 'stop_presenting'
                    return SpeakerTimelineEvent(
...
class SpeakerTimelineManager:
    ...
    def add_speaker(self, speaker_info: SpeakerInfo) -> bool:
        ...
        with self.lock:
            if speaker_info.speaker_id not in self.speakers:
                ...
                logger.info(f"New speaker added: {speaker_info.display_name} ({speaker_info.speaker_id})")
    ...
    async def _store_participant_data(self, speaker_info: SpeakerInfo):
        ...
        try:
            participant_data = {
                ...
                'join_time': datetime.fromtimestamp(speaker_info.join_time) if speaker_info.join_time else None,
                'leave_time': datetime.fromtimestamp(speaker_info.leave_time) if speaker_info.leave_time else None,
    ...
    def get_speaker_timeline(self, start_time: float = None, end_time: float = None) -> List[Dict]:
    ...
    def get_speaker_segments(self, speaker_id: str = None, start_time: float = None, end_time: float = None) -> List[Dict]:
...
# Factory functions
def create_caption_processor(session_id: str, bot_manager=None, database_manager=None) -> GoogleMeetCaptionProcessor:
