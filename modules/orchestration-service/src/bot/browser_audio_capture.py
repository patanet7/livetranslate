...
class BrowserAudioCapture:
    ...
    def __init__(self, config: BrowserAudioConfig, orchestration_url: str = "http://localhost:3000"):
        self.config = config
    ...
    async def initialize(self, meeting_id: str, session_id: str, browser_driver: Optional[webdriver.Chrome] = None):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to send audio chunk: {response.status_code} - {response.text}")
                    return None
...
def create_browser_audio_capture(config: Optional[BrowserAudioConfig] = None, orchestration_url: str = "http://localhost:3000") -> BrowserAudioCapture:
