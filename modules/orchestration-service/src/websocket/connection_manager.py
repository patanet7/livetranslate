...
class EnterpriseConnectionManager:
    ...
    def __init__(self, 
        ...
        # Enterprise features
        self.connection_pool = weakref.WeakValueDictionary()  # Weak references to connections
        self.message_buffer: Dict[str, List[Dict]] = defaultdict(list)  # Session-based message buffering
        ...
        logger.info(f"Enterprise WebSocket manager initialized (max: {max_connections})")
    ...
    def handle_disconnect(self, request) -> Dict[str, Any]:
        ...
        try:
            ...
            if connection:
                logger.info(f"WebSocket disconnected: {sid} (duration: {connection.get_connection_duration()})")
                return {"status": "disconnected", "duration": str(connection.get_connection_duration())}
            else:
    ...
    def _handle_heartbeat(self, sid: str) -> Dict[str, Any]:
        ...
        if self._socketio:
            self._socketio.emit('heartbeat_response', {
                'server_time': time.time(),
                'status': 'alive'
            }, room=sid)
    ...
    def _handle_join_session(self, sid: str, session_id: Optional[str]) -> Dict[str, Any]:
    ...
    def _handle_service_request(self, sid: str, target_service: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """Handle service-specific request"""
        # This would route to the appropriate service through the API gateway
        # For now, just acknowledge
        logger.info(f"Service request from {sid} to {target_service}: {data.get('action')}")
        
        if self._socketio:
            self._socketio.emit('service_response', {
    ...
    def broadcast_to_session(self, session_id: str, message: Dict[str, Any]):
        ...
        with self._lock:
            ...
            if connections:
                ...
                logger.debug(f"Broadcasted to {len(connections)} connections in session {session_id}")
            else:
    ...
    def _cleanup_loop(self):
        ...
        while self._running:
            try:
                ...
                with self._lock:
                    empty_buffers = [session_id for session_id, msgs in self.message_buffer.items() if not msgs]
