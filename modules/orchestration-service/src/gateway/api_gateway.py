...
class APIGateway:
    ...
    def route_request(self, service_name: str, api_path: str, flask_request) -> Response:
        ...
        try:
            ...
            if not instance:
                ...
                return jsonify({"error": f"No healthy instances for {service_name}"}), 503
    ...
    def _select_service_instance(self, service_name: str) -> Optional[ServiceInstance]:
        ...
        with self._lock:
            ...
            healthy_instances = [inst for inst in instances if inst.status in [ServiceStatus.HEALTHY, ServiceStatus.UNKNOWN]]
    ...
    def _prepare_request_data(self, flask_request) -> Dict[str, Any]:
        ...
        if flask_request.method in ['POST', 'PUT', 'PATCH']:
            if flask_request.is_json:
                ...
                for key, file in flask_request.files.items():
                    ...
                    logger.info(f"Gateway read file {key}: {file.filename} ({len(file_content)} bytes, {file.content_type})")
                    
                    # Verify we actually read the content
                    if len(file_content) == 0:
                        logger.error(f"Gateway read 0 bytes from file {key} - file may be empty or already consumed")
                    ...
                    files[key] = (file.filename or 'upload', file_content, file.content_type or 'application/octet-stream')
                    logger.info(f"Gateway created file tuple for {key}: filename={file.filename}, size={len(file_content)}, type={file.content_type}")
                
                if files:
                    request_data['files'] = files
                else:
                    logger.error("Gateway: No valid files found despite flask_request.files being truthy")
                ...
                if flask_request.content_type and 'multipart/form-data' in flask_request.content_type:
                    logger.warning("Found multipart/form-data in raw data - Flask parsing may have failed")
        ...
        skip_headers = {'host', 'content-length', 'connection', 'upgrade', 'sec-websocket-key', 'content-type'}
    ...
    def _make_request_with_retries(self, method: str, url: str, request_data: Dict, 
                                  service_name: str, circuit_breaker: Optional[CircuitBreaker]) -> Response:
                ...
                if 'files' in request_data:
                    ...
                    for key, file_info in request_data['files'].items():
                        if isinstance(file_info, tuple) and len(file_info) >= 2:
                            ...
                            logger.info(f"Gateway converted file {key} to BytesIO: {filename} ({len(content)} bytes, {content_type})")
                        else:
        ...
        with self._lock:
            for service_name, instances in self.services.items():
                service_health = {
                    ...
                    "circuit_breaker": self.circuit_breakers.get(service_name, {}).state.value if service_name in self.circuit_breakers else "none"
                }
        ...
        with self._lock:
            return {
                **self.metrics,
                ...
                "requests_per_second": self.metrics["total_requests"] / max(time.time() - self.start_time, 1),
                "success_rate": (
