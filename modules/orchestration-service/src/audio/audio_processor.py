...
class AudioPipelineProcessor:
    ...
    def _initialize_stages(self):
        ...
        spectral_denoising_stage = SpectralDenoisingStage(
            self.config.spectral_denoising, self.sample_rate
        )
        conventional_denoising_stage = ConventionalDenoisingStage(
            self.config.conventional_denoising, self.sample_rate
        )

        # Enhanced stages (pyloudnorm, pedalboard)
        lufs_normalization_stage = LUFSNormalizationStage(
            self.config.lufs_normalization, self.sample_rate
        )
    ...
    def _set_performance_targets(self):
        ...
        performance_targets = {
            ...
            "conventional_denoising": StagePerformanceTarget(
                target_latency_ms=8.0, max_latency_ms=15.0
            ),
            "lufs_normalization": StagePerformanceTarget(
    ...
    def process_audio_chunk(
        ...
        try:
            ...
            if self.metrics_manager:
                try:
                    self.metrics_manager.store_pipeline_metrics(
                        pipeline_result, session_id, chunk_id
                    )
                except Exception as e:
            ...
            processing_metadata["bypassed"] = (
                len(processing_metadata.get("stages_with_errors", [])) > 0
            )
