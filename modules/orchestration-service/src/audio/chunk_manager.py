...
class AudioBuffer:
    ...
    def __init__(self, config: AudioChunkingConfig):
        ...
        logger.info(
            f"AudioBuffer initialized: chunk={self.chunk_samples}, overlap={self.overlap_samples}, buffer={self.buffer_samples}"
        )
...
class AudioQualityAnalyzer:
    ...
    def analyze_chunk(self, audio_data: np.ndarray, sample_rate: int = 16000) -> QualityMetrics:
        ...
        try:
            ...
            noise_estimate = (
                np.percentile(np.abs(audio_data), 10) ** 2
            )  # 10th percentile as noise estimate
            snr = (
                10 * np.log10(signal_power / max(noise_estimate, 1e-10))
                if signal_power > 0
                else -60
            )
...
class ChunkManager:
    ...
    async def stop(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "rejection_rate": self.chunks_rejected
            / max(self.chunks_processed + self.chunks_rejected, 1),
    ...
    async def _process_single_chunk(self, audio_data: np.ndarray, chunk_metadata: dict[str, Any]):
        ...
        try:
            ...
            if quality_metrics.overall_quality_score < self.config.min_quality_threshold:
                ...
                logger.debug(
                    f"Rejected chunk {chunk_metadata['chunk_sequence']} due to low quality: {quality_metrics.overall_quality_score:.3f}"
                )
                return
            ...
            audio_chunk_metadata = create_audio_chunk_metadata(
                ...
                overlap_duration=chunk_metadata.get("overlap_metadata", {}).get(
            ...
            if chunk_id:
                ...
                self.average_quality_score = (
                    self.average_quality_score * (self.chunks_processed - 1)
                ...
                logger.debug(
                    f"Processed chunk {chunk_metadata['chunk_sequence']}: quality={quality_metrics.overall_quality_score:.3f}, time={processing_time:.1f}ms"
                )
            else:
                self.chunks_rejected += 1
                logger.warning(
                    f"Failed to store chunk {chunk_metadata['chunk_sequence']} in database"
                )

        except Exception as e:
            self.chunks_rejected += 1
            logger.error(
                f"Failed to process chunk {chunk_metadata.get('chunk_sequence', 'unknown')}: {e}"
            )
            if self.on_error:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_audio_duration": self.total_audio_duration,
            "average_quality_score": self.average_quality_score,
            "current_processing_rate": len(self.processing_times)
            / max(1, sum(self.processing_times) / 1000)
            if self.processing_times
            else 0,
            "average_processing_time": np.mean(self.processing_times)
            if self.processing_times
...
async def main():
    ...
    try:
        ...
        chunk_manager = create_chunk_manager(
            config, db_adapter, "test-session-123", SourceType.BOT_AUDIO
        )
