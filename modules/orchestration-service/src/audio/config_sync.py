...
class ConfigurationValidator:
    ...
    def _validate_whisper_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "sample_rate" in config:
            ...
            if not isinstance(sample_rate, int) or sample_rate not in [
                8000,
                16000,
                22050,
                44100,
                ...
                result["errors"].append(
                    f"Invalid sample_rate: {sample_rate}. Must be one of: 8000, 16000, 22050, 44100, 48000"
                )
        ...
        if "inference_interval" in config and config["inference_interval"] > 10.0:
            result["suggestions"].append(
                "Consider reducing inference_interval for better real-time performance"
            )
...
class ConfigurationSyncManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if not compatibility_check["compatible"]:
                logger.warning(
                    f"Configuration compatibility issues found: {compatibility_check['issues']}"
                )
                ...
                try:
                    ...
                    async with session.get(whisper_url, timeout=5) as response:
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"Whisper service config request failed: {response.status}"
                            )
                except Exception as e:
    ...
    async def get_unified_configuration(self) -> Dict[str, Any]:
        ...
        if (
            ...
            or not self._orchestration_config
            or not self._translation_config
        ):
            await self._fetch_all_configurations()
    ...
    async def update_configuration(
        ...
        try:
            if component == "whisper":
                ...
                update_result = await self._update_translation_configuration(
                    config_updates, propagate
                )
                ...
                try:
                    ...
                    if self._translation_config:
                        ...
                        async with session.post(
                            translation_url, json=config_updates, timeout=10
                        ) as response:
                            if response.status == 200:
                    ...
                    if self._translation_config:
                        ...
                        result["success"] = True
                        logger.info(
                            "✅ Translation configuration updated in cache (service unavailable)"
                        )
    ...
    async def _update_unified_configuration(
        ...
        try:
            ...
            if orchestration_updates:
                orch_result = await self._update_orchestration_configuration(
                    orchestration_updates, False
                )
            ...
            if translation_updates:
                trans_result = await self._update_translation_configuration(
                    translation_updates, False
                )
    ...
    async def _detect_configuration_conflicts(
        ...
        try:
            ...
            current_configs = {
                ...
                "orchestration": asdict(self._orchestration_config)
                if self._orchestration_config
                else {},
