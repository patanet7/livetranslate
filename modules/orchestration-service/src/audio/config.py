...
class AudioConfigurationManager:
    ...
    def __init__(
        ...
        )  # Go up from src/audio to orchestration-service
        ...
        logger.info(
            f"AudioConfigurationManager initialized with config file: {self.config_file_path}"
        )
    ...
    async def _save_config_to_database(
        self, config: AudioProcessingConfig, config_name: str = "default"
    ):
    ...
    async def apply_preset(
        ...
        logger.info(
            f"Applied preset {preset_name} to {'session ' + session_id if session_id else 'default'}"
        )
        return True
    ...
    def get_config_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "noise_reduction": {
                ...
                "mode": {
                    "type": "enum",
                    "values": list(NoiseReductionMode),
                    "default": "moderate",
                },
...
async def main():
    ...
    config_manager = create_audio_config_manager(
        config_file_path="./audio_config.yaml", auto_reload=False
    )
    ...
    try:
        ...
        await config_manager.update_config_from_dict(
            {"noise_reduction": {"strength": 0.9}}, session_id="test-session-123"
        )
