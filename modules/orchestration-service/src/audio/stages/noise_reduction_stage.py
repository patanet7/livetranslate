...
class NoiseReductionStage(BaseAudioStage):
    ...
    def _spectral_subtraction(self, audio_data: np.ndarray, alpha: float = 0.7, voice_protection: bool = False) -> np.ndarray:
    ...
    def get_noise_profile(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "adaptation_counter": self.adaptation_counter,
            "noise_profile_shape": self.noise_profile.shape if self.noise_profile is not None else None,
            "noise_profile_mean": np.mean(self.noise_profile) if self.noise_profile is not None else 0.0,
            "noise_profile_std": np.std(self.noise_profile) if self.noise_profile is not None else 0.0
        }
