...
class SpectralDenoisingStage(BaseAudioStage):
    ...
    def _process_spectral_frame(self, frame: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        if self.prev_magnitude is not None:
            denoised_magnitude = (
                self.alpha_smooth * self.prev_magnitude
        ...
        noise_reduction_db = 20 * np.log10(
            np.mean(magnitude) / max(np.mean(denoised_magnitude), 1e-10)
        )
        spectral_floor_applied = np.sum(
            denoised_magnitude <= (magnitude * self.config.spectral_floor)
        )
    ...
    def _update_noise_estimation(self, magnitude: np.ndarray):
        ...
        if frame_energy < energy_threshold or self.noise_estimation_frames < 10:
            ...
            self.signal_power_est = 0.9 * self.signal_power_est + 0.1 * magnitude**2
