...
class ConventionalDenoisingStage(BaseAudioStage):
    ...
    def __init__(self, config: ConventionalDenoisingConfig, sample_rate: int = 16000):
        ...
        if config.high_freq_emphasis > 0:
            ...
            self.emphasis_b, self.emphasis_a = scipy.signal.butter(
    ...
    def _bilateral_filter_denoising(
        ...
        for i in range(len(audio_data)):
            ...
            spatial_weights = np.exp(
                -0.5 * ((np.arange(window_size) - half_window) ** 2) / (sigma_space**2)
            )
    ...
    def _wavelet_denoising(self, audio_data: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        try:
            ...
            coeffs = self.pywt.wavedec(
                audio_data, self.config.wavelet_type, level=self.config.wavelet_levels
            )
    ...
    def _rnr_filter_denoising(self, audio_data: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        if self.emphasis_b is not None:
            ...
            # Blend emphasis
            audio_with_emphasis = (
                1 - self.config.high_freq_emphasis
            ) * audio_data + self.config.high_freq_emphasis * emphasized
        else:
