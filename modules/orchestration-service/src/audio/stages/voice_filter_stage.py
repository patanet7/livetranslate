    ...
    def _process_audio(self, audio_data: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        try:
            ...
            metadata = {
                ...
                "fundamental_range_hz": [self.config.fundamental_min, self.config.fundamental_max],
                "formant1_range_hz": [self.config.formant1_min, self.config.formant1_max],
                "formant2_range_hz": [self.config.formant2_min, self.config.formant2_max],
        ...
        try:
            ...
            if self.fundamental_filter is not None:
                w, h = scipy.signal.freqz(
                    self.fundamental_filter[0], self.fundamental_filter[1], 
                    worN=frequencies, fs=self.sample_rate
                )
            ...
            if self.rolloff_filter is not None:
                w, h = scipy.signal.freqz(
                    self.rolloff_filter[0], self.rolloff_filter[1], 
                    worN=frequencies, fs=self.sample_rate
                )
