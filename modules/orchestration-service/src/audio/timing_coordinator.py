...
class TimeWindow:
    ...
    def overlaps_with(self, other: "TimeWindow", tolerance: float = 0.0) -> bool:
        ...
        return not (
            self.end_timestamp <= other.start_timestamp - tolerance
            or self.start_timestamp >= other.end_timestamp + tolerance
        )
...
class TimingCoordinator:
    ...
    async def detect_time_drift(
        ...
        try:
            ...
            reference_timestamps = await self._get_timestamps_for_source(
                session_id, reference_source
            )
    ...
    async def align_session_timestamps(
        self, session_id: str, force_realignment: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if not force_realignment:
                ...
                if quality and quality.overall_score > 0.9:
                    logger.info(
                        f"Session {session_id} already well-aligned (score: {quality.overall_score:.2f})"
                    )
                    return True
            ...
            for chunk in audio_chunks:
                ...
                for transcript in transcripts:
                    ...
                    if chunk_window.overlaps_with(transcript_window):
                        ...
                        accuracy = self._determine_timing_accuracy(
                            time_offset * 1000
                        )  # Convert to ms
                        ...
                        correlation = TimingCorrelation(
                            ...
                            correlation_metadata={
                                ...
                                "overlap_duration": chunk_window.intersection_with(
                                    transcript_window
                                ).duration
                                if chunk_window.intersection_with(transcript_window)
                                else 0.0,
                            },
                        )
    ...
    async def _apply_drift_correction(self, session_id: str, source: str, drift: float) -> bool:
        ...
        try:
            logger.info(
                f"Applying drift correction for {source} in session {session_id}: {drift}ms"
            )
    ...
    async def _load_timing_correlations(self, session_id: str) -> list[TimingCorrelation]:
        ...
        try:
            if not self.database_adapter or not hasattr(
                self.database_adapter, "get_timing_correlations"
            ):
                return []
...
def create_timing_coordinator(
    database_adapter=None, timing_tolerance_ms: float = 100.0, **kwargs
) -> TimingCoordinator:
