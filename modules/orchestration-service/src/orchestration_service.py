...
class OrchestrationService:
    ...
    async def start(self):
        ...
        try:
            ...
            self.running = True
            logger.info(
                f"Orchestration service started successfully on port {self.config['frontend']['port']}"
            )
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "configuration": {
                ...
                "websocket_max_connections": self.config["websocket"][
                    "max_connections"
                ],
                "health_check_interval": self.config["monitoring"][
                    "health_check_interval"
                ],
            },
            "timestamp": time.time(),
...
def create_flask_app(
    ...
    app.config["SECRET_KEY"] = orchestration_service.config.get("frontend", {}).get(
        "secret_key", "orchestration-secret-key"
    )
    CORS(app)
    ...
    @app.route(
        "/api/<service_name>/<path:api_path>", methods=["GET", "POST", "PUT", "DELETE"]
    )
    def gateway_proxy(service_name, api_path):
        ...
        try:
            ...
            if service_name in [
                ...
                return (
                    jsonify({"error": "Endpoint handled by orchestration service"}),
                    400,
                )
...
async def main():
    ...
    try:
        ...
        def metrics_update_loop():
            while True:
                try:
                    orchestration_service.dashboard.update_metrics(
        ...
        socketio.run(
            app, host=args.host, port=args.port, debug=args.debug, use_reloader=False
        )
