...
class HealthAlert:
    ...
    def __init__(self, service_name: str, level: AlertLevel, message: str, 
...
class ServiceHealthTracker:
    ...
    def record_health_check(self, success: bool, response_time: Optional[float] = None, 
        ...
        if success:
            ...
            if self.status == HealthStatus.UNHEALTHY and self.consecutive_successes >= self.recovery_threshold:
                self.status = HealthStatus.RECOVERING
            elif self.status == HealthStatus.RECOVERING and self.consecutive_successes >= self.recovery_threshold * 2:
...
class ServiceHealthMonitor:
    ...
    def __init__(self, health_check_interval: int = 10, alert_threshold: int = 3, 
                 recovery_threshold: int = 2, auto_recovery: bool = True):
        """
    ...
    def _handle_status_change(self, tracker: ServiceHealthTracker, success: bool, error: Optional[str] = None):
        ...
        if previous_status != tracker.status:
            logger.info(f"Service {tracker.name} status changed: {previous_status.value} -> {tracker.status.value}")
            ...
            elif tracker.status == HealthStatus.HEALTHY and previous_status in [HealthStatus.UNHEALTHY, HealthStatus.RECOVERING]:
                self._generate_alert(
                    tracker.name,
                    AlertLevel.INFO,
    ...
    def get_all_service_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            healthy_services = sum(1 for t in self.services.values() if t.status == HealthStatus.HEALTHY)
            degraded_services = sum(1 for t in self.services.values() if t.status == HealthStatus.DEGRADED)
            unhealthy_services = sum(1 for t in self.services.values() if t.status == HealthStatus.UNHEALTHY)
            ...
            return {
                ...
                "summary": {
                    ...
                    "overall_uptime": (
                        sum(t.uptime_percentage for t in self.services.values()) / max(total_services, 1)
                    )
                },
                "timestamp": time.time()
            }
