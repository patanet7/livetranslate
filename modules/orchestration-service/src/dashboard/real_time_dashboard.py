...
class MetricsCollector:
    ...
    def get_metric_history(self, name: str, duration_seconds: Optional[int] = None) -> List[Dict]:
...
class RealTimeDashboard:
    ...
    def update_metrics(self, orchestration_service):
        ...
        try:
            ...
            if hasattr(orchestration_service, 'websocket_manager'):
                ...
                self.metrics_collector.record_metric("active_connections", ws_stats.get("active_connections", 0))
                self.metrics_collector.record_metric("total_connections", ws_stats.get("total_connections", 0))
                self.metrics_collector.record_metric("active_sessions", ws_stats.get("active_sessions", 0))
            
            # API Gateway metrics
            if hasattr(orchestration_service, 'api_gateway'):
                gateway_metrics = orchestration_service.api_gateway.get_metrics()
                self.metrics_collector.record_metric("total_requests", gateway_metrics.get("total_requests", 0))
                self.metrics_collector.record_metric("avg_response_time", gateway_metrics.get("average_response_time", 0))
    ...
    def _update_dashboard_data(self, orchestration_service):
        ...
        try:
            ...
            self.dashboard_data["system_overview"] = {
                ...
                "status": "running",
                "performance_score": self.performance_analyzer.analyze_performance().get("performance_score", 100),
                "timestamp": time.time()
            }
            ...
            # Service status
            if hasattr(orchestration_service, 'health_monitor'):
                self.dashboard_data["service_status"] = orchestration_service.health_monitor.get_all_service_status()
            
            # Performance analysis
            analysis = self.performance_analyzer.analyze_performance()
            self.dashboard_data["alerts"] = analysis.get("warnings", []) + analysis.get("critical_issues", [])
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "data_points": sum(len(history) for history in self.metrics_collector.metrics_history.values())
        }
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_data_points": sum(len(history) for history in self.metrics_collector.metrics_history.values()),
            "performance_score": self.performance_analyzer.analyze_performance().get("performance_score", 100)
        }
