...
const config = {
    // Use orchestration service endpoints
    serverUrl: window.location.origin, // Current orchestration service URL
    whisperApiUrl: '/api/whisper', // Proxied through orchestration gateway
    translationApiUrl: '/api/translation', // Proxied through orchestration gateway
    healthApiUrl: '/api/health',
    servicesApiUrl: '/api/services',
...
function setupEventListeners() {
    ...
    elements.refreshStatus?.addEventListener('click', () => API.checkServerHealth(false));
    elements.refreshModels?.addEventListener('click', API.loadModels);
    ...
    elements.selectedModel?.addEventListener('change', function() {
        ...
        UI.updateModelSelection(this.value);
        addLog('INFO', `Model changed to: ${this.value}`);
    });
...
async function initializeApp() {
    ...
    try {
        ...
        // Check server health
        await API.checkServerHealth(false);
...
async function testTranscribeAudio() {
    ...
    try {
        ...
        formData.append('audio', state.audioTest.recordedBlob, 'audio.wav');
...
window.WhisperApp = {
    selectModel: function(modelName) {
        ...
        UI.updateModelSelection(modelName);
...
// Make functions available globally for inline onclick handlers and backwards compatibility
...
// Add global wrappers for API and UI functions to maintain backwards compatibility
// Wait for API and UI to be loaded
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof UI !== 'undefined') {
        window.updateModelsDisplay = UI.updateModelsDisplay;
        window.updateServerStatus = UI.updateServerStatus;
    }
});
