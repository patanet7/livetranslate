...
# Load audio files
jfk_audio, jfk_sr = librosa.load('../whisper-service/tests/fixtures/audio/jfk.wav', sr=16000, mono=True)
chinese_audio, cn_sr = librosa.load('../whisper-service/tests/fixtures/audio/OSR_cn_000_0072_8k.wav', sr=None, mono=True)
...
print(f"  {len(jfk_part1)/16000:.2f}-{(len(jfk_part1)+len(chinese_part1))/16000:.2f}s: Chinese (ZH)")
print(f"  {(len(jfk_part1)+len(chinese_part1))/16000:.2f}-{(len(jfk_part1)+len(chinese_part1)+len(jfk_part2))/16000:.2f}s: JFK (EN)")
print(f"  {(len(jfk_part1)+len(chinese_part1)+len(jfk_part2))/16000:.2f}-{(len(jfk_part1)+len(chinese_part1)+len(jfk_part2)+len(chinese_part2))/16000:.2f}s: Chinese (ZH)")
print(f"  {(len(jfk_part1)+len(chinese_part1)+len(jfk_part2)+len(chinese_part2))/16000:.2f}-{len(mixed_audio)/16000:.2f}s: JFK (EN)")
