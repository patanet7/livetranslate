...
down_revision: str | None = "002_session_id_nullable"
...
def upgrade() -> None:
    ...
    if not column_exists(connection, "glossary_entries", "phonetic"):
        op.add_column("glossary_entries", sa.Column("phonetic", sa.String(255), nullable=True))
    ...
    if not column_exists(connection, "glossary_entries", "common_context"):
        op.add_column("glossary_entries", sa.Column("common_context", sa.Text(), nullable=True))
    ...
    if table_exists(connection, "domain_usage_logs"):
        # Drop indexes first
        for idx in [
            "ix_domain_usage_session",
            "ix_domain_usage_logs_created_at",
            "ix_domain_usage_domain_date",
            "ix_domain_usage_domain",
    ...
    if table_exists(connection, "domain_prompts"):
        # Drop indexes first
        for idx in [
            "ix_domain_prompts_usage",
            "ix_domain_prompts_domain",
    ...
    if table_exists(connection, "domain_terminology"):
        # Drop indexes first
        for idx in [
            "ix_domain_terminology_term",
            "ix_domain_terminology_normalized",
            "ix_domain_terminology_importance",
    ...
    if table_exists(connection, "domain_categories"):
        # Drop indexes first
        for idx in [
            "ix_domain_categories_usage",
            "ix_domain_categories_name",
...
def downgrade() -> None:
    ...
    if not table_exists(connection, "domain_categories"):
        ...
        op.create_index(
            "ix_domain_categories_active", "domain_categories", ["is_active"], unique=False
        )
        op.create_index(
            op.f("ix_domain_categories_name"), "domain_categories", ["name"], unique=True
        )
        op.create_index(
    ...
    if not table_exists(connection, "domain_usage_logs"):
        ...
        op.create_index(
            "ix_domain_usage_created", "domain_usage_logs", ["created_at"], unique=False
        )
        ...
        op.create_index(
            op.f("ix_domain_usage_logs_created_at"),
            "domain_usage_logs",
            ["created_at"],
            unique=False,
        )
        op.create_index(
            "ix_domain_usage_session", "domain_usage_logs", ["session_id"], unique=False
        )
