...
export class GoogleMeetBot extends MeetBotBase {
  ...
  async join({ url, name, bearerToken, teamId, timezone, userId, eventId, botId, uploader, orchestrationUrl }: JoinParams): Promise<void> {
    ...
    try {
      ...
      await this.joinMeeting({ url, name, bearerToken, teamId, timezone, userId, eventId, botId, uploader, pushState, orchestrationUrl });
  ...
  private async joinMeeting({ url, name, teamId, userId, eventId, botId, pushState, uploader, orchestrationUrl }: JoinParams & { pushState(state: BotStatus): void }): Promise<void> {
    ...
    // CRITICAL: Dismiss Google Sign In popup FIRST
    const dismissGoogleSignInPopup = async () => {
      try {
        ...
        this._logger.warn('Error checking for Google Sign In popup', { error: error.message });
      }
    };

    await dismissGoogleSignInPopup();
    ...
    // Set up audio streaming if orchestrationUrl is provided
    if (orchestrationUrl && botId && userId) {
      this._logger.info('Setting up audio streaming', { orchestrationUrl, botId });

      try {
        // Create and connect audio streamer
        ...
        this._logger.info('Audio streamer connected successfully');

        // Expose function to receive audio chunks from browser
        await this.page.exposeFunction('streamAudioChunk', async (audioBase64: string) => {
          if (this.audioStreamer) {
            const audioBuffer = Buffer.from(audioBase64, 'base64');
            await this.audioStreamer.streamAudio(audioBuffer);
          }
        });

        // Inject audio capture code into the browser
        await this.page.evaluate(async () => {
          console.log('[LiveTranslate] Starting audio-only capture...');

          try {
            // Capture AUDIO ONLY from Google Meet tab
            const stream: MediaStream = await (navigator.mediaDevices as any).getDisplayMedia({
              video: false,  // NO VIDEO - AUDIO ONLY
              audio: {
            ...
            if (audioTracks.length === 0) {
              console.error('[LiveTranslate] No audio tracks available');
              return;
            }

            console.log('[LiveTranslate] Audio tracks captured:', audioTracks.length);
            ...
            processor.onaudioprocess = async (e) => {
              ...
              await (window as any).streamAudioChunk(base64);
            };

            console.log('[LiveTranslate] Audio streaming started successfully');
          } catch (error) {
            console.error('[LiveTranslate] Failed to start audio capture:', error);
          }
        });

        this._logger.info('Audio capture injected successfully');
      } catch (error: any) {
    ...
    // Only record video if NOT using audio streaming
    if (!orchestrationUrl) {
      this._logger.info('Begin recording...');
      await this.recordMeetingPage({ teamId, eventId, userId, botId, uploader });
      pushState('finished');
    } else {
      this._logger.info('Audio streaming mode - skipping video recording');
      // Keep the page open for audio streaming
      // The bot will stay in 'joined' state until explicitly stopped
    }
  }
  ...
  private async recordMeetingPage(
    ...
    waitingPromise.promise.then(async () => {
      ...
      // Close audio streamer if active
      if (this.audioStreamer) {
        this._logger.info('Closing audio streamer...');
        await this.audioStreamer.close();
  ...
  /**
   * Leave the meeting and cleanup resources
   */
  async leave(): Promise<void> {
    try {
      ...
      this._logger.info('Meeting left successfully');
    } catch (error: any) {
      this._logger.error('Error leaving meeting', { error: error.message });
      throw error;
    }
  }

  /**
   * Get bot state (for status endpoint)
   */
  getState(): string {
    return this.audioStreamer ? 'streaming' : 'joined';
  }
}
