    ...
    try:
        with wave.open(JFK_AUDIO_PATH, "rb") as wav_file:
            ...
            if max_amplitude < 0.001:
                ...
                print("   ✅ Audio has good signal level")

            # Check sample rate compatibility
            if frame_rate != 16000:
                print(f"\n⚠️ Sample rate is {frame_rate}Hz, Whisper expects 16kHz")
                print("   Will need resampling")
            else:
...
def test_direct_transcription(jfk_audio):
    ...
    try:
        ...
        print("   Using: model_manager.safe_inference() (same as streaming)")
        ...
        if result:
            ...
            if segments:
                ...
                for i, seg in enumerate(segments[:5]):  # Show first 5
                    if isinstance(seg, dict):
                        print(
                            f"   {i+1}. [{seg.get('start', 0):.1f}s - {seg.get('end', 0):.1f}s] '{seg.get('text', '')}'"
                        )
                    else:
                        print(
                            f"   {i+1}. [{getattr(seg, 'start', 0):.1f}s - {getattr(seg, 'end', 0):.1f}s] '{getattr(seg, 'text', '')}'"
                        )
            ...
            if len(found_keywords) >= 2:
                print(
                    f"   ✅ JFK speech detected! Found {len(found_keywords)}/{len(expected_keywords)} keywords"
                )
                return True
            else:
                print(
                    f"   ❌ JFK speech NOT detected. Found only {len(found_keywords)}/{len(expected_keywords)} keywords"
                )
                print("   ⚠️ This means the issue is NOT in the streaming pipeline!")
                print("   ⚠️ The JFK file itself may not contain the expected speech")
                return False
        else:
