...
class TestAudioGenerator:
    ...
    @staticmethod
    def generate_sine_wave(duration: float = 1.0, frequency: int = 440, sample_rate: int = 16000) -> np.ndarray:
    ...
    @classmethod
    def save_test_audio(cls, audio_data: np.ndarray, filename: str, sample_rate: int = 16000) -> str:
...
def assert_response_time(response_time: float, max_time: float, operation: str = "operation"):
    ...
    assert response_time <= max_time, f"{operation} took {response_time:.3f}s, expected <= {max_time}s"

def assert_websocket_message(message: Dict[str, Any], expected_type: str, required_fields: List[str] = None):
...
async def wait_for_condition(condition: Callable, timeout: float = 10, interval: float = 0.1) -> bool:
