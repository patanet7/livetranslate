...
class TestLongSession60Minutes:
    ...
    def test_60_minute_continuous_streaming(self, transcriber):
        ...
        try:
            for batch_idx in range(int(total_duration_sec / batch_duration)):
        ...
        assert memory_growth_mb < max_acceptable_growth_mb, \
        ...
        assert mem_stats['peak_mb'] < memory_limit_mb * 1.5, \
...
class TestKVCacheOverflow:
    ...
    def test_kv_cache_bounded_in_long_session(self, transcriber):
        ...
        for i in range(0, len(audio), chunk_size):
            ...
            if i % (sample_rate * 60) == 0:  # Every minute
                ...
                logger.info(
                    f"[{i // (sample_rate * 60)}min] Memory: {sample['total_mb']:.1f} MB"
                )
        ...
        assert mem_stats['growth_mb'] < 100, \
    ...
    def test_session_restart_prevents_cache_overflow(self, transcriber):
        ...
        for i in range(5):  # 5 speech segments
            # Speech
    ...
    def test_session_history_bounded_growth(self, transcriber):
        ...
        for i in range(num_sessions):
            ...
            if (i + 1) % 10 == 0:
                ...
                logger.info(
                    f"Sessions: {i+1}, Memory: {sample['total_mb']:.1f} MB"
                )
        ...
        logger.info(f"✅ Session history bounded: {memory_growth_mb:+.1f} MB growth for {num_sessions} sessions")
