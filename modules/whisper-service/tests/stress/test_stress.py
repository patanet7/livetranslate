...
class TestConnectionStress:
    ...
    @pytest.mark.asyncio
    @pytest.mark.stress
    async def test_high_connection_count(
        self, test_config, stress_test_config, performance_monitor
    ):
...
class TestMessageStress:
    ...
    async def test_high_message_throughput(
        self, websocket_client, stress_test_config, performance_monitor
    ):
    ...
    async def test_large_message_handling(self, websocket_client):
        ...
        for size in sizes:
            large_data = "x" * size
            message = {"type": "test_large", "data": large_data, "size": size}
    ...
    async def test_concurrent_message_streams(self, test_config, stress_test_config):
        ...
        tasks = [send_message_stream(client, i) for i, client in enumerate(clients)]
...
class TestAudioStress:
    ...
    async def test_concurrent_audio_streams(self, test_config, test_audio_sine_wave):
        ...
        start_time = time.time()
        tasks = [send_audio_stream(client, i) for i, client in enumerate(clients)]
...
class TestErrorStress:
    ...
    async def test_rapid_invalid_messages(self, websocket_client):
        ...
        for _i in range(20):  # Reduced for CI
    ...
    async def test_connection_failure_recovery(self, test_config):
        ...
        for _i in range(recovery_attempts):
            client = WebSocketTestClient(test_config["websocket_url"])
...
class TestPerformanceBenchmarks:
    ...
    async def test_baseline_performance_benchmark(
        self, test_config, benchmark_thresholds, performance_monitor
    ):
