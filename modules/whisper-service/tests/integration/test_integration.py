...
class TestHTTPEndpoints:
    ...
    async def test_health_endpoint(
        self, http_client, sample_api_endpoints, expected_response_schemas
    ):
...
class TestPerformanceIntegration:
    ...
    async def test_connection_performance(
        ...
        assert connected is True
        assert_response_time(
            connection_time, benchmark_thresholds["connection_time"], "WebSocket connection"
        )
    ...
    @pytest.mark.asyncio
    async def test_message_response_performance(
        self, websocket_client, benchmark_thresholds, performance_monitor
    ):
        ...
        assert response is not None
        assert_response_time(
            response_time, benchmark_thresholds["message_response_time"], "message response"
        )
        ...
        async with http_client as client:
            ...
            if response.status == 200:
                ...
                if data["models"]:
                    ...
                    assert transcribe_response.status in [
                        200,
                        404,
                    ]  # Either works or not implemented
