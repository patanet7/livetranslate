...
def _ensure_streaming_session(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if session_id not in streaming_sessions:
        ...
        streaming_sessions[session_id].update(
            {
                "model_name": data.get(
                    "model_name", streaming_sessions[session_id].get("model_name", "whisper-base")
                ),
            ...
            async def stream_to_client():
                try:
                    async for result in whisper_service.transcribe_stream(transcription_request):
                        ...
                        try:
                            socketio.emit(
                                "transcription_result", transcription_data, room=request.sid
                            )
                        except Exception as emit_error:
                            # If emit fails, try to buffer the message for reconnection
                            logger.warning(
                                f"Failed to emit transcription result to {request.sid}: {emit_error}"
                            )
                            session_info = reconnection_manager.get_session_by_connection(
                                request.sid
                            )
                            if session_info:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to emit transcription complete to {request.sid}: {emit_error}"
                        )
