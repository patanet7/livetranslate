...
from .whisper.audio import N_FRAMES, N_SAMPLES, TOKENS_PER_SECOND, log_mel_spectrogram, pad_or_trim
from .whisper.decoding import BeamSearchDecoder, GreedyDecoder, SuppressTokens
...
class PaddedAlignAttWhisper:
    def __init__(self, cfg: AlignAttConfig) -> None:
        ...
        # Enhanced device detection: CUDA > MPS > CPU
        # SimulStreaming original only supported CUDA/CPU, we're adding MPS support
        import torch

        if torch.cuda.is_available():
            device = "cuda"
            logger.info("[Device] Using CUDA device")
        elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            device = "mps"
            logger.info("[Device] Using MPS (Metal Performance Shaders) device")
        else:
            device = "cpu"
            logger.info("[Device] Using CPU device")
        ...
        for _i, b in enumerate(self.model.decoder.blocks):
        ...
        suppress_tokens = [
            ...
            *self.tokenizer.all_language_tokens,  # added by DM
        ]
    ...
    def set_task(self, task: str, language: str | None = None, enable_code_switching: bool = False):
        """
        Dynamically update task and regenerate initial tokens for new session

        This allows the same model instance to handle different tasks per-session:
        - task='transcribe': Same-language transcription (e.g., EN audio → EN text)
        - task='translate': Translation to English (e.g., FR audio → EN text)
        ...
        if enable_code_switching:
            logger.info("[SET_TASK] Code-switching enabled - will detect language on every chunk")
        ...
        # Handle 'auto' language detection (same pattern as __init__ line 67)
        # If language='auto', pass None to tokenizer (will be detected during infer)
        # If language=specific code (e.g., 'en'), use that
        # If language=None, keep current tokenizer language
        if language == "auto":
            tokenizer_language = None  # Will be detected during first infer()
            self.detected_language = None  # Reset detected language
            logger.info("[SET_TASK] Language set to 'auto' - will be detected during inference")
        elif language:
            tokenizer_language = language
            self.detected_language = language
            logger.info(f"[SET_TASK] Language set to '{language}'")
        else:
            tokenizer_language = (
                self.tokenizer.language if hasattr(self.tokenizer, "language") else None
            )
            logger.info(f"[SET_TASK] Keeping current tokenizer language: {tokenizer_language}")

        # Recreate tokenizer with new task and language
        self.create_tokenizer(tokenizer_language)

        # Regenerate initial_tokens with new task token
        self.init_tokens()

        logger.info(f"[SET_TASK] Updated initial_tokens: {self.initial_tokens[0].tolist()}")
        logger.info(
            f"[SET_TASK] Task token: {'<|transcribe|>' if task == 'transcribe' else '<|translate|>'}"
        )
    ...
    def trim_context(self):
        ...
        text_len = sum(t.shape[1] for t in self.tokens) + c
        #        logger.debug(f"len {text_len}, c {c}, max_context_tokens {self.max_context_tokens}")
        after = 0 if self.cfg.static_init_prompt is None else len(self.cfg.static_init_prompt)
        #        logger.debug(f"len {text_len}, c {c}, max_context_tokens {self.max_context_tokens}")
        while c > self.max_context_tokens or text_len > self.max_text_len - 20:
            ...
            if t == 0:
                break
        #        logger.debug(f"len {text_len}, c {c}, max_context_tokens {self.max_context_tokens}")
        logger.info(f"Context after trim: {self.context.text} (len: {text_len})")
    ...
    def _current_tokens(self):
        ...
        # make it one tensor
        current_tokens = torch.cat(toks, dim=1) if len(toks) > 1 else toks[0]
    ...
    def infer(self, is_last=False):
        ...
        should_detect_language = self.cfg.language == "auto" and (
        ...
        if should_detect_language:
            ...
            language_switched = (
                self.detected_language is not None and self.detected_language != top_lan
            )

            if language_switched:
                logger.info(
                    f"🔄 Language switch detected: {self.detected_language} → {top_lan} (p={p:.4f})"
                )
            else:
            ...
            if self.detected_language is None:
                ...
                self.detected_language = top_lan
                logger.info(
                    f"[CODE-SWITCHING] Language tracked as {top_lan} but decoder unchanged (no cache flush)"
                )
            else:
                # Standard mode: language shouldn't change once set
                logger.warning(
                    f"Language changed from {self.detected_language} to {top_lan} but not in code-switching mode"
                )
        ...
        while not completed and current_tokens.shape[1] < self.max_text_len:  # bos is 3 tokens
            ...
            if (
                not is_last
                and self.last_attend_frame - most_attended_frame > self.cfg.rewind_threshold
            ):
                # TODO: check this
                if current_tokens.shape[1] > 1 and current_tokens[0, -2] >= DEC_PAD:
                    ...
                    self.last_attend_frame = -self.cfg.rewind_threshold
                    current_tokens = (
                        torch.cat(self.tokens, dim=1) if len(self.tokens) > 0 else self.tokens[0]
                    )
                    break
            else:
                self.last_attend_frame = most_attended_frame
    ...
    def logdir_save(self, input_segments, new_hypothesis, generation):
        ...
        with open(os.path.join(dir, f"iter_{self.logdir_i:05d}_hypothesis.txt"), "w") as f:
            context = generation["starting_tokens"].as_text(self.tokenizer) if generation else ""
