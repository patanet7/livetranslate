...
class ModelManager:
    ...
    def __init__(self, models_dir: str | None = None, use_hf_pipeline: bool | None = None):
        ...
        logger.info(
            "Note: Whisper model deprecation warnings are suppressed. Consider updating to newer OpenVINO model formats."
        )
        ...
        if model_name not in self.pipelines:
            ...
            if not os.path.exists(model_path):
                ...
                if available_models:
                    ...
                    raise FileNotFoundError(
                        f"No models found in {self.models_dir}. Please mount models directory."
                    )
            ...
            try:
                if self.use_hf_pipeline:
                    ...
                    logger.info(
                        f"[MODEL] ✅ Model {model_name} loaded successfully on {device} in {load_time:.2f}s"
                    )
                else:
                ...
                logger.info(
                    f"[MODEL] 📊 Total loaded models: {len(self.pipelines)} ({list(self.pipelines.keys())})"
                )
                ...
                if self.use_hf_pipeline:
                    ...
                    try:
                        ...
                        self.pipelines[model_name] = pipe
                        logger.info(
                            f"[MODEL] ✅ Model {model_name} loaded on CPU in {fallback_time:.2f}s"
                        )
                    ...
                    try:
                        ...
                        with warnings.catch_warnings():
                            ...
                            warnings.filterwarnings(
                                "ignore",
                                message=".*Whisper decoder models with past is deprecated.*",
                            )
                            start_fallback_time = time.time()
            ...
            try:
                ...
                try:
                    ...
                    if "Infer Request is busy" in error_msg:
                        ...
                        raise Exception(
                            "Device is busy processing another request. Please try again."
                        ) from device_error
                        ...
                        or "insufficient host memory" in error_msg
                    ):
                        ...
                        if "large" in model_name:
                            raise Exception(
                                "Out of memory. Try using whisper-tiny or whisper-base instead of large models."
                            ) from device_error
                        else:
                            raise Exception(
                                "Out of memory. Cache cleared - please try again with fewer concurrent requests."
                            ) from device_error
    ...
    async def transcribe(self, request: TranscriptionRequest) -> TranscriptionResult:
        ...
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                None, self.model_manager.safe_inference, request.model_name, audio_data
            )
        ...
        try:
            ...
            while self.streaming_active:
                ...
                if len(new_audio) > 0:  # Process if we have new audio
                    try:
                        logger.info(
                            f"[STREAM] Processing {len(new_audio)} new samples ({len(new_audio)/request.sample_rate:.2f}s)"
                        )
                        ...
                        logger.info(
                            f"[STREAM] ✅ Transcribed new audio: '{result.text[:50]}...' (Lang: {result.language})"
                        )
