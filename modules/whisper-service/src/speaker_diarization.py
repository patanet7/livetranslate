...
class SpeechEnhancer:
    ...
    def enhance_audio(
        self, audio: np.ndarray, background_noise_sample: np.ndarray | None = None
    ) -> np.ndarray:
                ...
                try:
                    if background_noise_sample is not None:
                        enhanced = nr.reduce_noise(
                            y=enhanced, sr=self.sample_rate, y_noise=background_noise_sample
                        )
                    else:
...
class SpeakerClusterer:
    ...
    def __init__(self, method: str = "hdbscan", n_speakers: int | None = None):
        self.method = method.lower()
    ...
    def _cluster_hdbscan(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        clusterer = hdbscan.HDBSCAN(
            min_cluster_size=max(2, len(embeddings) // 10), min_samples=1, metric="cosine"
        )
    ...
    def _cluster_dbscan(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        clusterer = DBSCAN(eps=0.3, min_samples=max(1, len(embeddings) // 20), metric="cosine")
        labels = clusterer.fit_predict(embeddings)
    ...
    def _cluster_agglomerative(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        clusterer = AgglomerativeClustering(
            n_clusters=n_clusters, linkage="average", affinity="cosine"
        )
        labels = clusterer.fit_predict(embeddings)
...
class AdvancedSpeakerDiarization:
    ...
    def __init__(self, config: DiarizationConfig | None = None):
        ...
        self.speech_enhancer = (
            SpeechEnhancer(self.config.sample_rate) if self.config.enable_enhancement else None
        )
        self.embedding_extractor = EmbeddingExtractor(
            self.config.embedding_method, self.config.device
        )
    ...
    def process_audio_chunk(
        self, audio: np.ndarray, timestamp: float | None = None
    ) -> list[SpeakerSegment]:
        ...
        try:
            with self.lock:
                ...
                embedding = self.embedding_extractor.extract_embedding(
                    enhanced_audio, self.config.sample_rate
                )
    ...
    def _identify_speaker(self, embedding: np.ndarray) -> int:
        ...
        if max_similarity > self.config.similarity_threshold:
            ...
            self.speaker_profiles[best_speaker] = (1 - alpha) * self.speaker_profiles[
                best_speaker
            ] + alpha * embedding
            return best_speaker
        else:
...
class PunctuationAligner:
    ...
    def align_segments_with_punctuation(
        self, segments: list[SpeakerSegment], text: str, duration: float
    ) -> list[SpeakerSegment]:
...
def quick_speaker_diarization(
    audio: np.ndarray, sample_rate: int = 16000, n_speakers: int | None = None
) -> list[SpeakerSegment]:
