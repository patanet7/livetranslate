...
class HeartbeatManager:
    ...
    def __init__(self, 
        ...
        self.on_connection_warning: Optional[Callable[[str, HeartbeatInfo], None]] = None
    ...
    def start(self):
        ...
        with self._lock:
            ...
            logger.info(f"Heartbeat manager started (ping_interval={self.ping_interval}s, "
    ...
    def handle_pong(self, connection_id: str, ping_timestamp: Optional[str] = None):
        ...
        with self._lock:
            ...
            logger.debug(f"Received pong from {connection_id} (missed: {heartbeat.missed_heartbeats})")
    ...
    def get_connection_health(self, connection_id: str) -> Optional[HeartbeatInfo]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_rtt = 0.0
            if self.heartbeats:
                rtts = [h.get_average_rtt() for h in self.heartbeats.values() if h.get_average_rtt() > 0]
    ...
    def _cleanup_disconnected_connections(self):
        ...
        with self._lock:
            ...
            for connection_id, heartbeat in self.heartbeats.items():
                if (heartbeat.state == HeartbeatState.DISCONNECTED and 
                    now - heartbeat.last_pong > cleanup_threshold):
