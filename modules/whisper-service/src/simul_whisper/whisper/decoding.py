    ...
    language_probs = [
        {
            ...
            for j, c in zip(
                tokenizer.all_language_tokens, tokenizer.all_language_codes, strict=False
            )
        }
...
class MaximumLikelihoodRanker(SequenceRanker):
    ...
    def rank(self, tokens: list[list[Tensor]], sum_logprobs: list[list[float]]):
        ...
        return [np.argmax(scores(p, lens)) for p, lens in zip(sum_logprobs, lengths, strict=False)]
...
class BeamSearchDecoder(TokenDecoder):
    ...
    def update(self, tokens: Tensor, logits: Tensor, sum_logprobs: Tensor) -> tuple[Tensor, bool]:
        ...
        for i in range(n_audio):
            ...
            for j in range(self.beam_size):
                ...
                for logprob, token in zip(*logprobs[idx].topk(self.beam_size + 1), strict=False):
                    new_logprob = (sum_logprobs[idx] + logprob).item()
                    sequence = (*prefix, token.item())
        ...
        for previously_finished, newly_finished in zip(
            self.finished_sequences, finished_sequences, strict=False
        ):
    ...
    def finalize(self, preceding_tokens: Tensor, sum_logprobs: Tensor):
        ...
        tokens: list[list[Tensor]] = [
            [torch.tensor(seq) for seq in sequences] for sequences in self.finished_sequences
        ]
...
class DecodingTask:
    ...
    def run(self, mel: Tensor) -> list[DecodingResult]:
        ...
        tokens: list[list[int]] = [t[i].tolist() for i, t in zip(selected, tokens, strict=False)]
