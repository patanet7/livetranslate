...
class PaddedAlignAttWhisper:
    ...
    def infer(self, is_last=False):
        ...
        should_detect_language = (
            ...
            is None  # Only detect ONCE (removed code-switching condition)
        )
        ...
        if should_detect_language:
            ...
            logger.info(
                f"✅ Language pinned to '{top_lan}' for session duration (SimulStreaming baseline behavior)"
            )
        ...
        while not completed and current_tokens.shape[1] < self.max_text_len:  # bos is 3 tokens
            ...
            last_token_id = (
                current_tokens[0, -1].item()
                if self.decoder_type == "beam"
                else current_tokens[:, -1].item()
            )
            last_token_text = self.tokenizer.decode([last_token_id])
            logger.info(
                f"[TOKEN-DEBUG] Generated token #{current_tokens.shape[1]}: ID={last_token_id}, Text='{last_token_text}', Lang={self.detected_language}, SOT={self.tokenizer.language}"
            )
