...
class PaddedAlignAttWhisper:
    def __init__(self, cfg: AlignAttConfig) -> None:
        ...
        if torch.cuda.is_available():
            ...
            device = "mps"
            logger.info(f"[Device] Using MPS (Metal Performance Shaders) device")
        else:
            device = "cpu"
    ...
    def set_task(self, task: str, language: str = None, enable_code_switching: bool = False):
        ...
        if language == 'auto':
            ...
            logger.info(f"[SET_TASK] Language set to 'auto' - will be detected during inference")
        elif language:
            tokenizer_language = language
            self.detected_language = language
            logger.info(f"[SET_TASK] Language set to '{language}'")
        else:
            tokenizer_language = self.tokenizer.language if hasattr(self.tokenizer, 'language') else None
        ...
        logger.info(f"[SET_TASK] Task token: {'<|transcribe|>' if task == 'transcribe' else '<|translate|>'}")
