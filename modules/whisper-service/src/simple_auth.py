...
class SimpleAuth:
    ...
    def authenticate(self, user_id: str, password: str) -> AuthToken | None:
    ...
    def get_connection_auth(self, connection_id: str) -> AuthToken | None:
    ...
    def check_permission(self, token: str, required_role: UserRole) -> bool:
        ...
        role_levels = {UserRole.GUEST: 0, UserRole.USER: 1, UserRole.ADMIN: 2}
    ...
    def cleanup_expired_tokens(self):
        ...
        expired_tokens = [
            token for token, auth_token in self.tokens.items() if auth_token.is_expired()
        ]
...
def auth_middleware(context):
    ...
    if context.route_info:
        ...
        if not simple_auth.check_permission(auth_token.token, required_role):
            logger.warning(
                f"Insufficient permissions for {context.connection_id} "
