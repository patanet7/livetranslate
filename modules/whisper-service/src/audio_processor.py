...
class AudioProcessor:
    ...
    def detect_format(self, audio_data: bytes) -> str:
        ...
        if (
            b"webm" in first_100.lower()
            or b"opus" in first_100.lower()
            or b"vorbis" in first_100.lower()
        ):
            return "webm"
    ...
    def process_audio(
        self, audio_data: bytes, target_sample_rate: int | None = None, enhance: bool | None = None
        ...
        for strategy in strategies:
            try:
                ...
                if audio is not None and len(audio) > 0:
                    logger.debug(
                        f"✓ Audio processed with {strategy.__name__}: {len(audio)} samples at {sr}Hz"
                    )
    ...
    def _process_with_librosa(
        self, audio_data: bytes, format_hint: str, target_sr: int
    ) -> tuple[np.ndarray, int]:
    ...
    def _process_with_pydub(
        ...
        try:
            ...
            raise AudioConversionError(f"Pydub processing failed: {e}") from e

    def _process_raw_audio(
        self, audio_data: bytes, format_hint: str, target_sr: int
    ) -> tuple[np.ndarray, int]:
    ...
    def _temp_audio_file(self, audio_data: bytes, format_hint: str):
        ...
        try:
            with tempfile.NamedTemporaryFile(
                suffix=extension, dir=self.temp_dir, delete=False
            ) as temp_file:
    ...
    def convert_format(
        self, audio_data: bytes, target_format: str = "wav", target_sample_rate: int | None = None
    ) -> bytes:
        """
...
def process_audio_data(
    audio_data: bytes, sample_rate: int = 16000, enhance: bool = True
) -> tuple[np.ndarray, int]:
    """Quick audio processing function."""
    processor = AudioProcessor(default_sample_rate=sample_rate, enable_enhancement=enhance)
