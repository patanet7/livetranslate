...
from connection_manager import ConnectionState, connection_manager
...
from utils.audio_errors import (
    ...
    MemoryError,
    ModelLoadingError,
    ValidationError as WhisperValidationError,
    WhisperProcessingBaseError,
...
async def initialize_service():
    ...
    try:
        ...
        logger.info(
            "Whisper service, connection manager, heartbeat manager, and authentication system initialized successfully"
        )
    except Exception as e:
...
async def process_orchestration_chunk():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"error": f"Invalid base64 audio data: {e!s}"}), 400
        ...
        transcription_request = TranscriptionRequest(
            ...
            enable_vad=chunk_metadata.get(
                "enable_vad", False
            ),  # VAD already applied by orchestration
        ...
        if hasattr(whisper_service, "process_orchestration_chunk"):
            ...
            if response_data.get("status") == "success":
                logger.info(
                    f"[WHISPER] 📝 Transcription result: {response_data['transcription']['text'][:100]}..."
                )
            else:
                logger.error(
                    f"[WHISPER] ❌ Chunk processing failed: {response_data.get('error', 'Unknown error')}"
                )
        else:
            # Fallback to legacy processing
            logger.warning(
                "[WHISPER] Using legacy processing method - orchestration features unavailable"
            )
            ...
            logger.info(
                f"[WHISPER] ✅ Chunk {chunk_id} processed in {processing_time:.2f}s (legacy)"
            )
...
async def transcribe_with_model(model_name: str):
    ...
    with error_boundary(
        ...
        try:
            logger.info(
                f"[WHISPER] [{correlation_id}] 🎤 Transcription request received for model: {model_name}"
            )
            ...
            if (
                whisper_service.model_manager
                and model_name not in whisper_service.model_manager.models
            ):
                logger.info(
                    f"[WHISPER] 🔄 Model {model_name} not loaded, will load on first use..."
                )
            else:
            ...
            logger.info(
                f"[WHISPER] 📁 Request files keys: {list(request.files.keys()) if request.files else 'None'}"
            )
            logger.info(
                f"[WHISPER] 📝 Request form keys: {list(request.form.keys()) if request.form else 'None'}"
            )
            logger.info(
                f"[WHISPER] 📊 Request data length: {len(request.data) if request.data else 0}"
            )
            ...
            if request.content_type and "multipart/form-data" in request.content_type:
                ...
                if "audio" not in request.files:
                    logger.error(
                        f"No 'audio' field in files. Available fields: {list(request.files.keys())}"
                    )
                ...
                logger.error(
                    f"Unsupported request format. Content-Type: {request.content_type}, Files: {bool(request.files)}, Data: {len(request.data) if request.data else 0}"
                )
                return jsonify(
                    {
            ...
            enable_vad = (
                request.form.get("enable_vad", "false").lower() == "true" if request.form else False
            )  # Temporarily disable VAD to debug

            # Process audio
            logger.info(f"[WHISPER] 🔊 Processing audio data: {len(audio_data)} bytes")
            audio_array = _process_audio_data(audio_data)
            logger.info(
                f"[WHISPER] 📊 Audio processed to array: {audio_array.shape if hasattr(audio_array, 'shape') else len(audio_array)} samples"
            )
            ...
            if hasattr(audio_array, "shape") and len(audio_array) > 0:
                ...
                logger.info(
                    f"[WHISPER] 🎵 Audio duration: {duration:.3f}s, RMS: {rms_level:.6f}, Max amp: {max_amplitude:.3f}"
                )
            ...
            if result and result.text:
                ...
                if len(words) > 15:  # Only check longer texts
                    ...
                    if repetition_ratio < 0.15:  # Less than 15% unique words (was 10%)
                        logger.warning(
                            f"[WHISPER] Detected extreme repetition - ratio: {repetition_ratio:.2f}"
                        )
                        ...
                        logger.info(
                            f"[WHISPER] Adjusted confidence to {result.confidence_score:.3f} due to repetition"
                        )
...
async def process_pipeline_endpoint():
    ...
    try:
        ...
        model_name = request.form.get(
            "model", whisper_service.config.get("default_model", "whisper-base")
        )
        session_id = request.form.get("session_id")
...
def configure_streaming():
    ...
    try:
        ...
        return jsonify(
            {"status": "configured", "session_id": session_id, "config": streaming_config}
        )
...
def _ensure_streaming_session(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if session_id not in streaming_sessions:
        ...
        streaming_sessions[session_id].update(
            {
                "model_name": data.get(
                    "model_name", streaming_sessions[session_id].get("model_name", "large-v3-turbo")
                ),
                "language": data.get("language", streaming_sessions[session_id].get("language")),
                "buffer_duration": data.get(
                    "buffer_duration", streaming_sessions[session_id].get("buffer_duration", 6.0)
                ),
                "inference_interval": data.get(
                    "inference_interval",
                    streaming_sessions[session_id].get("inference_interval", 3.0),
                ),
                "enable_vad": data.get(
                    "enable_vad", streaming_sessions[session_id].get("enable_vad", True)
                ),
            }
        )
...
def start_streaming():
    ...
    try:
        ...
        return jsonify({"status": "streaming_started", "session_id": session_id, "config": config})
...
def get_rolling_transcriptions():
    ...
    try:
        ...
        if session_id:
            ...
            transcriptions = session.get("transcriptions", [])[-limit:] if session else []
        else:
...
def get_session(session_id: str):
    ...
    try:
        ...
        return jsonify(
            {"session_id": session_id, "session": session, "timestamp": datetime.now().isoformat()}
        )
...
def api_download_result(request_id: str):
    ...
    # Transcript manager is not yet implemented
    # When implemented, this will return stored transcripts
    return jsonify(
        {
...
def get_service_status():
    ...
    try:
        ...
        model_info = {
            "device": whisper_service.model_manager.device
            if whisper_service.model_manager
            else "unknown",
            "loaded_models": list(whisper_service.model_manager.pipelines.keys())
            if whisper_service.model_manager
            else [],
            "available_models": whisper_service.get_available_models(),
            "default_model": whisper_service.config.get("default_model", "whisper-base"),
            "models_directory": whisper_service.config.get("models_dir"),
            "total_loaded": len(whisper_service.model_manager.pipelines)
            if whisper_service.model_manager
            else 0,
        }
        ...
        logger.info(
            f"[STATUS] 📊 Service status requested - Device: {model_info['device']}, Loaded: {model_info['loaded_models']}"
        )
        ...
        return jsonify(
            {
                ...
                "active_streams": len(
                    [s for s in streaming_sessions.values() if s.get("streaming_active", False)]
                ),
...
def get_connections():
    ...
    try:
        ...
        return jsonify({"connections": connections, "timestamp": datetime.now().isoformat()})
    except Exception as e:
...
def get_error_statistics():
    ...
    try:
        ...
        response = {"statistics": stats, "timestamp": datetime.now().isoformat()}
    ...
    try:
        ...
        if detailed:
            ...
            response["connection_health"] = {
                conn_id: heartbeat.to_dict() for conn_id, heartbeat in all_health.items()
            }

            unhealthy = heartbeat_manager.get_unhealthy_connections()
            response["unhealthy_connections"] = {
                conn_id: heartbeat.to_dict() for conn_id, heartbeat in unhealthy.items()
            }
...
def get_session_details(session_id: str):
    ...
    try:
        ...
        if session_info:
            return jsonify(
                {"session": session_info.to_dict(), "timestamp": datetime.now().isoformat()}
            )
        else:
            return jsonify({"error": "Session not found"}), 404
    except Exception as e:
...
def get_performance_statistics():
    ...
    try:
        ...
        pool_stats = {
            ...
            "max_workers": audio_pool.executor._max_workers
            if hasattr(audio_pool.executor, "_max_workers")
            else "unknown",
        }
        ...
        return jsonify(
            {
                "performance_overview": {
                    "avg_audio_processing_time": perf_stats.get("audio_processing_times", {}).get(
                        "average", 0
                    ),
                    "avg_transcription_time": perf_stats.get("transcription_times", {}).get(
                        "average", 0
                    ),
                    "avg_websocket_message_time": perf_stats.get("websocket_message_times", {}).get(
                        "average", 0
                    ),
                    "active_connections": conn_stats.get("active_connections", 0),
                    "pool_efficiency": conn_stats.get("performance_metrics", {}).get(
                ...
                "optimization_features": {
                    "connection_pooling": conn_stats.get("configuration", {}).get(
...
def handle_connection_warning(connection_id: str, heartbeat_info):
    ...
    logger.warning(
        f"Connection warning for {connection_id}: {heartbeat_info.state.value} "
...
def handle_connect():
    ...
    try:
        ...
        error_info = create_connection_error(
            "Failed to establish WebSocket connection", str(e), request.sid
        )
        error_handler.handle_error(error_info)
        emit("error", error_info.to_websocket_response()["error"])
        return False
...
def handle_disconnect():
    ...
    if connection_info:
        ...
        if session_info:
            logger.info(
                f"Client disconnected: {request.sid} (duration: {duration}, session: {session_info.session_id} preserved for reconnection)"
            )
        else:
...
def handle_join_session(data):
    ...
    if session_id:
        ...
        streaming_sessions[session_id] = streaming_config
        logger.info(
            f"[JOIN_SESSION] Stored config for {session_id}: enable_code_switching={streaming_config.get('enable_code_switching')}, model={streaming_config.get('model_name')}"
        )
        ...
        if connection_manager.join_session(request.sid, session_id):
            ...
            logger.info(
                f"Client {request.sid} joined session {session_id} with model {streaming_config['model_name']}"
            )
            emit(
                "joined_session",
                {
    ...
    try:
        ...
        try:
            ...
            connection_manager.update_connection_activity(
                request.sid, bytes_received=len(audio_bytes)
            )
            ...
            error_info = create_audio_error(
                "Failed to process audio data", str(e), data.get("session_id")
            )
            error_info.connection_id = request.sid
            error_handler.handle_error(error_info)
            emit("error", error_info.to_websocket_response()["error"])
        ...
        session_id = data.get("session_id")
        config = (
            data.get("config", streaming_sessions.get(session_id, {}))
            if session_id
        ...
        logger.info(
            f"[WS DEBUG] target_language in data: {data.get('target_language', 'NOT_PRESENT')}"
        )
        ...
        transcription_request = TranscriptionRequest(
            ...
            enable_code_switching=config.get(
                "enable_code_switching", data.get("enable_code_switching", False)
            ),
            ...
            sustained_lang_duration=config.get(
                "sustained_lang_duration", data.get("sustained_lang_duration")
            ),
            sustained_lang_min_silence=config.get(
                "sustained_lang_min_silence", data.get("sustained_lang_min_silence")
            ),
        ...
        logger.info(
            f"[WS DEBUG] Created TranscriptionRequest: task={transcription_request.task}, target_language={transcription_request.target_language}"
        )
        ...
        with vac_processors_lock:
            if session_id not in vac_processors:
                ...
                vac = VACOnlineASRProcessor(
                    ...
                    vad_min_silence_ms=transcription_request.vad_min_silence_ms
                    or 500,  # Match SimulStreaming!
                ...
                logger.info(
                    f"[VAC]   enable_code_switching={transcription_request.enable_code_switching}"
                )
        ...
        try:
            ...
            logger.debug(
                f"[VAC] Session {session_id}: Inserted audio chunk ({len(audio_array)} samples)"
            )
        except Exception as e:
            ...
            async def stream_to_client():
                try:
                    ...
                    if session_id in whisper_service.session_stability_trackers:
                        ...
                        try:
                            tokenizer = (
                                whisper_service.model_manager.current_model.tokenizer
                                if hasattr(whisper_service.model_manager.current_model, "tokenizer")
                                else None
                            )
                        except Exception:
                            tokenizer = None
                        ...
                        tracker = StabilityTracker(
                            config=whisper_service.stability_config, tokenizer=tokenizer
                        )
                    ...
                    while session_id in vac_processors:
                        ...
                        if text == last_emitted_text:
                            logger.debug(
                                f"[VAC] Session {session_id}: No change, skipping emission"
                            )
                            continue
                        ...
                        logger.info(
                            f"[VAC] Session {session_id}: New transcription - '{text[:50]}...' (final={is_final})"
                        )
                        ...
                        if tracker:
                            ...
                            stable_prefix = (
                                text  # For now, consider all text stable (VAC already filtered)
                            )
                            unstable_tail = ""
                            ...
                            result.should_translate = (
                                len(stable_prefix.split())
                                >= whisper_service.stability_config.min_stable_words
                            )
                            result.translation_mode = (
                                "final"
                                if is_final
                                else ("draft" if result.should_translate else "none")
                            )
                        ...
                        try:
                            socketio.emit(
                                "transcription_result", transcription_data, room=client_id
                            )
                        except Exception as emit_error:
                            # If emit fails, try to buffer the message for reconnection
                            logger.warning(
                                f"Failed to emit transcription result to {client_id}: {emit_error}"
                            )
                    ...
                    socketio.emit(
                        "transcription_error",
                        {"error": str(e), "session_id": transcription_request.session_id},
                        room=client_id,
                    )
        ...
        error_info = create_system_error("WebSocket streaming failed", str(e))
        error_info.connection_id = request.sid
...
def handle_heartbeat(data):
    ...
    try:
        ...
        emit(
            "heartbeat_ack", {"timestamp": datetime.now().isoformat(), "connection_id": request.sid}
        )
...
def handle_route_message(data):
    ...
    try:
        ...
        if result:
            emit(
                "route_response",
                {"type": message_type, "result": result, "timestamp": datetime.now().isoformat()},
            )

    except Exception as e:
        logger.error(f"Error routing message from {request.sid}: {e}")
        emit("error", {"message": f"Routing error: {e!s}"})
...
def handle_subscribe_events(data):
    ...
    try:
        result = handle_subscribe_events_message(
            MessageContext(
                connection_id=request.sid, message_type=MessageType.SUBSCRIBE_EVENTS, data=data
            )
        )
...
def handle_unsubscribe_events(data):
    ...
    try:
        result = handle_unsubscribe_events_message(
            MessageContext(
                connection_id=request.sid, message_type=MessageType.UNSUBSCRIBE_EVENTS, data=data
            )
        )
...
def handle_authenticate(data):
    ...
    try:
        ...
        if success:
            ...
            if session_info:
                reconnection_manager.update_session_data(
                    session_info.session_id,
                    {"user_id": auth_token.user_id, "role": auth_token.role.value},
                )
...
AUDIO_CONFIG = {
    ...
    "max_cache_size": 50,
    "quality_thresholds": {"silence_rms": 0.0001, "quiet_rms": 0.005, "clipping_threshold": 0.99},
}
...
def _detect_audio_format_optimized(audio_data: bytes) -> str:
    ...
    # MP3 format detection (ID3 tag or frame sync)
        ...
        or audio_data.startswith(b"\xff\xfa")
    ):
        format_hint = "mp3"
...
def _high_quality_resample(
    audio: np.ndarray, orig_sr: int, target_sr: int, quality: str = "kaiser_fast"
) -> np.ndarray:
...
def _calculate_audio_quality_metrics(audio: np.ndarray, sr: int) -> dict:
    ...
    metrics["dynamic_range"] = (
        float(metrics["peak"] - np.min(np.abs(audio[audio != 0]))) if np.any(audio != 0) else 0.0
    )
    ...
    metrics["is_clipped"] = (
        metrics["peak"] >= AUDIO_CONFIG["quality_thresholds"]["clipping_threshold"]
    )
...
def _process_audio_data(
    ...
    try:
        ...
        if len(audio_data) % 2 == 0 and len(audio_data) < 100000:  # Likely a streaming chunk
            ...
            if not (has_wav_header or has_mp3_header or has_ogg_header or has_flac_header):
                ...
                try:
                    ...
                    logger.info(
                        f"[AUDIO] Raw PCM processed: {len(audio_array)} samples at {current_sr}Hz"
                    )
                    ...
                    logger.debug(
                        f"[AUDIO] Raw PCM processing failed: {pcm_error}, trying file decode"
                    )
            ...
            if format_hint == "wav":
                ...
                try:
                    ...
                    with tempfile.NamedTemporaryFile(
                        suffix=f".{format_hint}", delete=False
                    ) as tmp_file:
                        tmp_file.write(audio_data)
                        tmp_file.flush()
                        audio_array, current_sr = librosa.load(
                            tmp_file.name, sr=None, dtype=np.float32
                        )
                        os.unlink(tmp_file.name)
                    processing_stages.append("librosa_file")
                logger.info(
                    f"[AUDIO] {format_hint.upper()} processing: {len(audio_array)} samples at {current_sr}Hz"
                )
                ...
                methods = [
                    ("soundfile", lambda: sf.read(io.BytesIO(audio_data), dtype=np.float32)),
                    (
                        "librosa_memory",
                        lambda: librosa.load(io.BytesIO(audio_data), sr=None, dtype=np.float32),
                    ),
                ]
        ...
        if quality_metrics["is_silent"]:
            ...
            logger.info(
                f"[AUDIO] Quiet audio detected (RMS: {quality_metrics['rms']:.6f}) - continuing"
            )
        ...
        logger.info(
            f"[AUDIO] Processing complete: {quality_metrics['duration']:.2f}s, "
            f"RMS: {quality_metrics['rms']:.4f}, Peak: {quality_metrics['peak']:.4f}"
        )
        logger.info(f"[AUDIO] Processing stages: {' -> '.join(processing_stages)}")
        logger.info(
            f"[AUDIO] Quality metrics: ZCR={quality_metrics['zero_crossing_rate']:.4f}, "
...
def _process_audio_async(audio_data: bytes, enhance: bool = False, priority: int = 5):
    ...
    future = audio_pool.submit_audio_task(
        _process_audio_data, priority=priority, audio_data=audio_data, enhance=enhance
    )
    return future
...
error_handler.register_recovery_handler(
    ErrorCategory.CONNECTION_FAILED, recover_whisper_connection_error
)
error_handler.register_recovery_handler(
    ErrorCategory.MODEL_LOAD_FAILED, recover_whisper_device_error
)
...
async def get_whisper_configuration():
    ...
    try:
        config_info = {
            ...
            if getattr(whisper_service, "orchestration_mode", False)
            else "legacy",
            ...
            "configuration": {
                ...
                "default_model": whisper_service.config.get("default_model", "whisper-base"),
                "max_concurrent_requests": whisper_service.config.get(
            ...
            "capabilities": {
                ...
                "buffer_management": hasattr(whisper_service, "buffer_manager")
                and whisper_service.buffer_manager is not None,
        ...
        compatibility_info = {
            ...
            "chunking_configuration": {
                ...
                "orchestration_compatible": {
                    ...
                    "overlap_duration": whisper_service.config.get("overlap_duration", 0.2),
                    "processing_interval": whisper_service.config.get("inference_interval", 3.0)
                    * 0.8,
                    "buffer_duration": whisper_service.config.get("buffer_duration", 4.0),
                },
            },
...
async def _validate_transcription_request(flask_request, model_name: str, correlation_id: str):
    ...
    try:
        ...
        if isinstance(e, WhisperProcessingBaseError):
            raise
        raise AudioCorruptionError(
            f"Failed to read audio file: {e!s}",
            correlation_id=correlation_id,
            corruption_details={"filename": audio_file.filename, "read_error": str(e)},
        ) from e
...
def _safe_model_loading(model_name: str, correlation_id: str):
    ...
    try:
        if (
            whisper_service.model_manager
            and model_name not in whisper_service.model_manager.pipelines
        ):
            logger.info(
                f"[WHISPER] [{correlation_id}] 🔄 Model {model_name} not loaded, loading now..."
            )
        ...
        if isinstance(e, WhisperProcessingBaseError):
            raise
        raise ModelLoadingError(
            f"Model loading failed: {e!s}",
            correlation_id=correlation_id,
            model_details={"model_name": model_name, "error": str(e)},
        ) from e
...
def _safe_audio_processing(audio_data: bytes, correlation_id: str):
    ...
    try:
        ...
        if "memory" in str(e).lower() or "allocation" in str(e).lower():
            raise MemoryError(
                f"Memory error during audio processing: {e!s}",
                correlation_id=correlation_id,
                memory_details={"audio_size": len(audio_data)},
            ) from e
        elif "cuda" in str(e).lower() or "gpu" in str(e).lower():
            raise HardwareError(
                f"GPU/CUDA error during audio processing: {e!s}",
                correlation_id=correlation_id,
                hardware_details={"device": "cuda"},
            ) from e
        else:
            raise AudioCorruptionError(
                f"Audio processing failed: {e!s}",
                correlation_id=correlation_id,
