...
class ContinuousStreamProcessor:
    ...
    def __init__(self, sample_rate: int = 16000, session_id: str | None = None):
        """
        ...
        self.previous_transcriptions = deque(
            maxlen=5
        )  # Last 5 transcriptions for overlap detection
    ...
    def add_chunk(self, audio_chunk: np.ndarray, chunk_metadata: dict | None = None) -> dict:
        """
        ...
        try:
            ...
            logger.debug(
                f"[{self.session_id}] Added chunk {self.total_chunks_received} "
    ...
    def _run_inference_with_deduplication(self, chunk_metadata: dict) -> dict:
        ...
        try:
            ...
            logger.info(
                f"[{self.session_id}] Running inference #{self.total_inferences_run} "
            ...
            logger.info(
                f'[{self.session_id}] Raw transcription: "{raw_text}" '
    ...
    def _deduplicate_text(self, new_text: str) -> str:
        ...
        for i, prev_text in enumerate(reversed(list(self.previous_transcriptions))):
            ...
            if overlap_length > 0:
                ...
                logger.debug(
                    f"[{self.session_id}] Found {overlap_length}-char overlap with previous text #{i}: "
    ...
    async def _send_to_translation_service(self, clean_text: str, language: str):
        ...
        try:
            ...
            logger.debug(
                f"[{self.session_id}] Sent clean text to translation service: '{clean_text[:30]}...'"
            )
...
class TranslationServiceClient:
    ...
    async def send_clean_text(self, text: str, session_id: str, metadata: dict | None = None):
        """
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/api/process_clean_text", json=payload, timeout=10.0
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Translation service error: {response.status_code} {response.text}"
                    )
                    return None
