...
class SessionInfo:
    ...
    def clear_delivered_messages(self, delivered_ids: List[str]):
        ...
        self.buffered_messages = [
            m for m in self.buffered_messages 
            if m.message_id not in delivered_ids
        ]
...
class ReconnectionManager:
    ...
    def create_session(self, connection_id: str, user_id: Optional[str] = None, 
    ...
    def handle_disconnection(self, connection_id: str) -> Optional[SessionInfo]:
    ...
    def attempt_reconnection(self, connection_id: str, session_id: str, 
        ...
        with self.lock:
            ...
            logger.info(f"Successfully reconnected session {session_id} to connection {connection_id}")
            return True
    
    def buffer_message(self, session_id: str, message_type: str, data: Any, 
                      priority: int = 0) -> bool:
    ...
    def get_session_by_connection(self, connection_id: str) -> Optional[SessionInfo]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            disconnected_sessions = len([s for s in self.sessions.values() if s.state == SessionState.DISCONNECTED])
