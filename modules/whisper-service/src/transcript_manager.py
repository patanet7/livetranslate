...
class TranscriptManager:
    ...
    def store_segment(self, text: str, session_id: str, metadata: dict[str, Any] | None = None) -> bool:
        ...
        try:
            ...
            with self.session_locks[session_id]:
                ...
                logger.debug(
                    f"[{session_id}] Stored segment: '{text[:50]}...' "
    ...
    def get_session_transcript(self, session_id: str, format: str = "text") -> str | None:
        """
    ...
    def get_recent_context(
        self, session_id: str, max_segments: int = 5, max_age_seconds: float = 300.0
    ...
    def _format_as_srt(self, segments: list[TranscriptSegment]) -> str:
        ...
        for i, segment in enumerate(segments, 1):
            ...
            lines.extend([str(i), f"{start_time} --> {end_time}", segment.text, ""])
            ...
            with self.manager_lock:
                ...
                if len(self.sessions) > self.max_sessions:
                    sorted_sessions = sorted(
                        self.session_stats.items(), key=lambda x: x[1].last_updated
                    )
...
# Factory functions
def create_transcript_manager(storage_dir: str | None = None, **kwargs) -> TranscriptManager:
