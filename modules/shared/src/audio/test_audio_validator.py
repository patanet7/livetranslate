...
class AudioValidatorTester:
    ...
    def create_test_file(
        self, audio_data: np.ndarray, file_format: AudioFormat, sample_rate: int = 16000
    ) -> str:
    ...
    def test_quality_assessment(self) -> dict[str, Any]:
        ...
        for scenario_name, audio in test_scenarios:
            quality_score, quality_level, metrics = self.validator.validate_audio_quality(
    ...
    def test_format_conversion(self) -> dict[str, Any]:
        ...
        for target_format in target_formats:
            try:
                ...
                results[target_format.value] = {"success": False, "error": str(e)}
    ...
    def test_file_operations(self) -> dict[str, Any]:
        ...
        for file_format in file_formats:
            try:
                ...
                results[file_format.value] = {"success": False, "error": str(e)}
    ...
    def test_convenience_functions(self) -> dict[str, Any]:
        ...
        try:
            converted_bytes, metadata = convert_audio_format(
                audio, AudioFormat.WAV, self.sample_rate
            )
    ...
    def run_all_tests(self) -> dict[str, Any]:
        ...
        try:
            ...
            all_results["error"] = f"Test suite failed: {e!s}"
    ...
    def generate_test_summary(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        summary = {
            "total_test_categories": len([k for k in results if k != "test_summary"]),
        ...
        if "corruption_detection" in results:
            ...
            summary["key_findings"].append(
                f"Corruption detection: {len(detected_corruptions)} types detected"
            )

        if "quality_assessment" in results:
            quality_results = results["quality_assessment"]
            quality_scores = [
                v.get("quality_score", 0) for v in quality_results.values() if isinstance(v, dict)
            ]
            if quality_scores:
            ...
            if isinstance(category_results, dict):
                for test_name, test_result in category_results.items():
                    if isinstance(test_result, dict):
                        status = (
                            "✅"
                            if test_result.get("success", test_result.get("is_valid", True))
                            else "❌"
                        )
