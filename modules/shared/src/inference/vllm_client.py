...
class VLLMClient(BaseInferenceClient):
    ...
    async def generate(self, request: InferenceRequest) -> InferenceResponse:
        ...
        try:
            ...
            raise InferenceTimeoutError("vLLM request timed out") from e
        except aiohttp.ClientError as e:
            raise BackendUnavailableError(f"Failed to connect to vLLM server: {e}") from e
        except Exception as e:
            logger.error(f"vLLM inference error: {e}")
            raise InferenceClientError(f"vLLM inference failed: {e}") from e
    ...
    async def generate_stream(
        ...
        try:
            ...
            raise InferenceTimeoutError("vLLM stream request timed out") from e
        except aiohttp.ClientError as e:
            raise BackendUnavailableError(f"Failed to connect to vLLM server: {e}") from e
        except Exception as e:
            logger.error(f"vLLM streaming error: {e}")
            raise InferenceClientError(f"vLLM streaming failed: {e}") from e
    ...
    def _parse_openai_response(
        self, response: dict[str, Any], request: InferenceRequest, latency_ms: float
    ) -> InferenceResponse:
        """
        ...
        try:
            ...
            raise InferenceClientError(f"Failed to parse vLLM response: {e}") from e
...
def create_vllm_client(
    ...
    """
    return VLLMClient(base_url=base_url, api_key=api_key, default_model=default_model)
