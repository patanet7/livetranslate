...
class SlidingLIDDetector:
    ...
    def add_detection(self, language: str, confidence: float, audio_position: float):
        ...
        logger.debug(
            f"[SlidingLID] Added: {detection}, window has {len(self.detections)} detections"
        )
    ...
    def _purge_old_detections(self):
        ...
        if before_count != after_count:
            logger.debug(
                f"[SlidingLID] Purged {before_count - after_count} old detections, "
    ...
    def get_current_language(self) -> str | None:
        ...
        logger.debug(
            f"[SlidingLID] Current language: {language} ({count}/{len(self.detections)} detections)"
        )

        return language

    def get_sustained_language(self, min_duration: float) -> str | None:
        """
        ...
        languages = {d.language for d in self.detections}
        if len(languages) > 1:
        ...
        if duration >= min_duration:
            logger.debug(
                f"[SlidingLID] Sustained language: {sustained_language} "
                f"({duration:.2f}s >= {min_duration}s)"
            )
            return sustained_language
        else:
            logger.debug(
                f"[SlidingLID] Language {sustained_language} not sustained "
                f"({duration:.2f}s < {min_duration}s)"
            )
            return None
    ...
    def get_window_stats(self) -> dict:
        ...
        if not self.detections:
            return {"detection_count": 0, "languages": [], "window_size": self.window_size}
