...
class VACOnlineASRProcessor:
    ...
    def __init__(
        ...
        self.current_sustained_language: str | None = None  # Track sustained language
        ...
        self.silence_start_time: float | None = None  # Track silence duration
        self.sustained_language_threshold: float = (
            2.5  # Minimum duration for sustained detection (2.5-3.0s)
        )
    ...
    def insert_audio_chunk(
        ...
        if vad_result is not None:
            if "start" in vad_result:
                ...
                logger.debug(
                    f"VAD: Speech END detected (sample {vad_result['end']}), silence timer started"
                )
            ...
            if self.status == "voice":
                ...
                if len(self.audio_buffer) > 0:
                    ...
                    self.audio_buffer = torch.tensor([], dtype=torch.float32)
                    logger.debug(
                        f"VAD: Speech continues - audio sent to processor (current_buffer_size: {self.current_online_chunk_buffer_size} samples)"
                    )
            else:
    ...
    def _should_reset_sot(self, detected_language: str | None) -> bool:
        """
        ...
        if detected_language != self.current_sustained_language:
            ...
            logger.info(
                f"[SUSTAINED_LID] Language change detected: "
        ...
        if language_duration < self.sustained_language_threshold:
            logger.debug(
                f"[SUSTAINED_LID] Language '{detected_language}' duration: "
        ...
        if self.silence_start_time is None:
            logger.debug("[SUSTAINED_LID] No VAD silence detected (silence_start_time=None)")
            return False

        silence_duration = current_time - self.silence_start_time
        if silence_duration < self.min_silence_for_reset:
            logger.debug(
                f"[SUSTAINED_LID] Silence duration: "
        ...
        if time_since_last_reset < self.sot_reset_cooldown:
            logger.info(
                f"[SUSTAINED_LID] SOT reset blocked by cooldown: "
                f"{time_since_last_reset:.2f}s < {self.sot_reset_cooldown}s"
            )
            return False

        # All conditions met!
        logger.info("[SUSTAINED_LID] ✅ SOT reset conditions MET:")
        logger.info(
            f"  - Language '{detected_language}' sustained for {language_duration:.2f}s (>= {self.sustained_language_threshold}s)"
        )
        logger.info(
            f"  - Silence duration: {silence_duration:.2f}s (>= {self.min_silence_for_reset}s)"
        )
        logger.info(
            f"  - Time since last reset: {time_since_last_reset:.2f}s (>= {self.sot_reset_cooldown}s)"
        )
    ...
    def _finish(self) -> dict[str, Any]:
        ...
        try:
            ...
            current_language = self.lid_detector.get_current_language()
            logger.info(
                f"[LID] FINAL - Per-chunk: {detected_language}, Window average: {current_language}"
            )
