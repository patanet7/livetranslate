...
const systemSlice = createSlice({
  ...
  reducers: {
        ...
        Object.entries(data.services).forEach(
          ([serviceName, serviceData]: [string, any]) => {
            if (serviceName in state.serviceHealth) {
              state.serviceHealth[
                serviceName as keyof SystemState["serviceHealth"]
              ] = {
                serviceName,
    ...
    updatePerformanceMetrics: (
      state,
      action: PayloadAction<Partial<SystemState["performance"]>>,
    ) => {
    ...
    updateCPUMetrics: (
      state,
      action: PayloadAction<SystemState["performance"]["cpu"]>,
    ) => {
    ...
    updateMemoryMetrics: (
      state,
      action: PayloadAction<SystemState["performance"]["memory"]>,
    ) => {
    ...
    updateNetworkMetrics: (
      state,
      action: PayloadAction<SystemState["performance"]["network"]>,
    ) => {
    ...
    updateApplicationMetrics: (
      state,
      action: PayloadAction<Partial<SystemState["applicationMetrics"]>>,
    ) => {
    ...
    decrementConnections: (state) => {
      state.applicationMetrics.activeConnections = Math.max(
        0,
        state.applicationMetrics.activeConnections - 1,
      );
    },
    ...
    updateConfiguration: (
      state,
      action: PayloadAction<Partial<SystemState["configuration"]>>,
    ) => {
    ...
    updateFeatureFlag: (
      ...
      state.configuration.features[action.payload.feature] =
        action.payload.enabled;
    },

    updateLimits: (
      state,
      action: PayloadAction<Partial<SystemState["configuration"]["limits"]>>,
    ) => {
    ...
    addAlert: (
      state,
      action: PayloadAction<
        Omit<SystemState["alerts"][0], "id" | "timestamp" | "acknowledged">
      >,
    ) => {
      const alert = {
    ...
    addLog: (
      state,
      action: PayloadAction<Omit<SystemState["logs"][0], "id" | "timestamp">>,
    ) => {
      const log = {
    ...
    updateStatistics: (
      state,
      action: PayloadAction<Partial<SystemState["statistics"]>>,
    ) => {
    ...
    updateSystemMetricsBulk: (
      ...
      if (action.payload.applicationMetrics) {
        Object.assign(
          ...
          action.payload.applicationMetrics,
        );
      }
