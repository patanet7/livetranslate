...
import { WebSocketMessage, WebSocketStats, WebSocketEventType } from "@/types";
...
const initialState: WebSocketState = {
  ...
  config: {
    ...
    reconnectInterval:
      Number(import.meta.env.VITE_WS_RECONNECT_INTERVAL) || 10000, // 10 seconds (was 5s - too aggressive)
    maxReconnectAttempts:
      Number(import.meta.env.VITE_WS_MAX_RECONNECT_ATTEMPTS) || 3, // Only 3 attempts before API fallback (was 10)
    heartbeatInterval:
      Number(import.meta.env.VITE_WS_HEARTBEAT_INTERVAL) || 45000, // 45 seconds (was 30s - more conservative)
...
const websocketSlice = createSlice({
  ...
  reducers: {
    ...
    initializeConnection: (
      state,
      action: PayloadAction<{ url?: string; protocols?: string[] }>,
    ) => {
      state.loading = true;
    ...
    connectionEstablished: (
      state,
      action: PayloadAction<{ connectionId: string; serverTime: number }>,
    ) => {
    ...
    connectionLost: (
      state,
      action: PayloadAction<{ reason: string; code?: number }>,
    ) => {
      ...
      if (state.stats.connectionDuration > 0) {
        state.stats.connectionDuration =
          Date.now() - state.stats.connectionDuration;
      }
    ...
    messageReceived: (state, action: PayloadAction<WebSocketMessage>) => {
      ...
      if (
        message.type === "connection:pong" &&
        state.connection.lastPingTime > 0
      ) {
    ...
    subscribe: (state, action: PayloadAction<WebSocketEventType>) => {
      ...
      state.subscriptions[eventType] =
        (state.subscriptions[eventType] || 0) + 1;
    },
    ...
    updateConfig: (
      state,
      action: PayloadAction<Partial<WebSocketState["config"]>>,
    ) => {
    ...
    addError: (
      state,
      action: PayloadAction<{ error: string; code?: number; context?: string }>,
    ) => {
      state.errors.push({
        timestamp: Date.now(),
        ...action.payload,
