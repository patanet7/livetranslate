...
export const useBotManager = () => {
  ...
  const { bots, activeBotIds, systemStats, loading, error } = useAppSelector(
    (state) => state.bot,
  );
  ...
  const spawnBot = useCallback(
    async (request: MeetingRequest): Promise<string> => {
      ...
      try {
        ...
        const botSpawnRequest = {
          config: {
            meeting_info: {
              ...
              meeting_title:
                request.meetingTitle || `Meeting ${request.meetingId}`,
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.detail || errorData.message || "Failed to spawn bot",
          );
        }
  ...
  const terminateBot = useCallback(
  ...
  const getBotStatus = useCallback(
    async (botId: string): Promise<BotInstance> => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.detail || errorData.message || "Failed to get bot status",
          );
        }
        ...
        const botUpdates: Partial<BotInstance> = {
          ...
          createdAt:
            rawBot.created_at || rawBot.createdAt || getCurrentISOTimestamp(),
          updatedAt: getCurrentISOTimestamp(),
          lastActiveAt:
            rawBot.last_activity ||
            rawBot.lastActiveAt ||
  ...
  const getActiveBots = useCallback(async (): Promise<BotInstance[]> => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.detail || errorData.message || "Failed to get active bots",
        );
      }
      ...
      const botsObject = processedBots.reduce(
        (acc: Record<string, Partial<BotInstance>>, bot: any) => {
          const processedBot: Partial<BotInstance> = {
            id: bot.bot_id || bot.botId,
            botId: bot.bot_id || bot.botId,
            status: bot.status || "active",
            createdAt:
              bot.created_at || bot.createdAt || getCurrentISOTimestamp(),
            updatedAt: getCurrentISOTimestamp(),
            lastActiveAt:
              bot.last_activity || bot.lastActiveAt || getCurrentISOTimestamp(),
      ...
      const activeBotIds = processedBots.map(
        (bot: any) => bot.bot_id || bot.botId,
      );
  ...
  const getSystemStats = useCallback(async (): Promise<SystemStats> => {
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          `Bot stats API failed with status ${response.status}, using default values`,
        );
        return {
          totalBotsSpawned: 0,
          activeBots: 0,
      ...
      const stats: SystemStats = {
        totalBotsSpawned:
          rawStats.total_bots_created || rawStats.totalBotsSpawned || 0,
        activeBots: rawStats.active_bots || rawStats.activeBots || 0,
        completedSessions:
          rawStats.total_bots_completed || rawStats.completedSessions || 0,
        errorRate: rawStats.recovery_rate
          ? (100 - rawStats.recovery_rate) / 100
          : rawStats.errorRate || 0,
        averageSessionDuration:
          rawStats.capacity_utilization || rawStats.averageSessionDuration || 0,
      };
  ...
  const refreshBots = useCallback(async (): Promise<void> => {
    ...
    try {
      await Promise.all([getActiveBots(), getSystemStats()]);
    } catch (error) {
      handleError(error);
    } finally {
      setLocalLoading(false);
  ...
  const exportBotData = useCallback(
    async (botId: string, format: "json" | "csv" = "json") => {
      try {
        const response = await fetch(
          `/api/bot/${botId}/export?format=${format}`,
        );
  ...
  const getBotLogs = useCallback(
    async (botId: string, limit: number = 100) => {
      try {
  ...
  const getBotTranslations = useCallback(
    async (botId: string, limit: number = 50) => {
      try {
        const response = await fetch(
          `/api/bot/${botId}/translations?limit=${limit}`,
        );

        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.message || "Failed to get bot translations",
          );
        }
