...
export const useErrorHandler = () => {
    ...
    if (error.status !== undefined) {
      ...
      if (error.status >= 400 && error.status < 500) {
        return {
          ...
          userMessage:
            error.data?.message || "There was an issue with your request.",
          details: error,
          timestamp,
          recoverable: true,
        };
  ...
  const reportError = useCallback(
    (appError: AppError, options: ErrorHandlerOptions) => {
  ...
  const handleError = useCallback(
    (error: any, options: ErrorHandlerOptions = {}) => {
      ...
      if (
        shouldReport &&
        (appError.type === "api" || appError.type === "unknown")
      ) {
      ...
      if (showNotification) {
        ...
        enqueueSnackbar(appError.userMessage || appError.message, {
          ...
          action:
            showRetry && appError.recoverable
              ? (key) => (
                  <button
                    onClick={() => {
                      closeSnackbar(key);
