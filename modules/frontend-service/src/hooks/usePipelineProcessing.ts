...
export const usePipelineProcessing = () => {
  ...
  const processPipeline = useCallback(
    async (request: PipelineProcessingRequest): Promise<any | null> => {
      try {
  ...
  const startRealtimeProcessing = useCallback(
    async (pipelineConfig: PipelineProcessingRequest["pipelineConfig"]) => {
      try {
        ...
        const response = await startRealtimeSessionAPI({
          pipelineConfig,
        }).unwrap();
   ...
   */
  const updateRealtimeConfig = useCallback(
    (stageId: string, parameters: Record<string, any>) => {
      if (
        websocketRef.current &&
        isRealtimeActive &&
        websocketRef.current.readyState === WebSocket.OPEN
