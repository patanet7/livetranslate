...
export const useWebSocket = () => {
  ...
  const handleMessage = useCallback(
    (message: WebSocketMessage) => {
      ...
      switch (message.type) {
        ...
        case "bot:spawned": {
          const spawnData = message.data as {
            botId: string;
            meetingId: string;
            status: any;
          };
          ...
          notifySuccess(
            "Bot Spawned",
            `Bot for meeting ${spawnData.meetingId} has been spawned successfully`,
          );
          break;
        }
        ...
        case "bot:translation_ready": {
          const translationData = message.data as {
            botId: string;
            translation: any;
          };
          dispatch(
        ...
        case "bot:error":
          if ("botId" in message.data && "error" in message.data) {
            ...
            notifyError(
              "Bot Error",
              `Bot ${errorData.botId}: ${errorData.error.errorMessage}`,
            );
          }
          break;
        ...
        case "bot:terminated":
          if ("botId" in message.data && "reason" in message.data) {
            const terminatedData = message.data as {
              botId: string;
              reason: string;
            };
            notifyInfo(
              "Bot Terminated",
              `Bot ${terminatedData.botId} has been terminated: ${terminatedData.reason}`,
            );
          }
          break;
        ...
        case "system:health_update":
          if ("services" in message.data) {
            const healthData = message.data as {
              services: any;
              timestamp: number;
            };
            ...
            if (Array.isArray(healthData.services)) {
              serviceHealth = healthData.services.reduce(
                (acc: any, service: any) => {
                  acc[
        ...
        case "system:performance_metrics":
          if ("cpu" in message.data && "memory" in message.data) {
            const perfData = message.data as {
              cpu: number;
              memory: number;
              timestamp: number;
            };
  ...
  const connect = useCallback(() => {
    ...
    if (wsFailureCount >= enhancedConfig.maxReconnectAttempts) {
      ...
      notifyWarning(
        ...
        "Switched to REST API mode for better stability",
      );
      return;
    }
    ...
    try {
      dispatch(
        initializeConnection({
          ...
          protocols: enhancedConfig.protocols,
        }),
      );

      websocketRef.current = new WebSocket(
        enhancedConfig.url,
        enhancedConfig.protocols,
      );
      ...
      websocketRef.current.onclose = (event) => {
        ...
        if (
          enhancedConfig.autoReconnect &&
          connection.reconnectAttempts < enhancedConfig.maxReconnectAttempts
        ) {
          ...
          const backoffDelay = Math.min(
            enhancedConfig.reconnectInterval * Math.pow(2, attemptNumber - 1),
            30000,
          );
  ...
  const sendMessageEnhanced = useCallback(
    <T extends WebSocketEventType>(
      type: T,
      data: WebSocketEventData<T>,
      correlationId?: string,
