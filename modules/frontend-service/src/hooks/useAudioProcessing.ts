...
export const useAudioProcessing = () => {
      ...
      if (result.stage_results && Array.isArray(result.stage_results)) {
        for (let i = 0; i < result.stage_results.length; i++) {
          ...
          setProcessingProgress(
            0.3 + (0.6 * (i + 1)) / result.stage_results.length,
          );
        ...
        throw new Error(
          `Invalid API response: expected 'stage_results' array but received: ${JSON.stringify(Object.keys(result))}`,
        );
      }
    ...
    async (audioBlob?: Blob) => {
      ...
      try {
        ...
        if (!pipelineResponse.ok) {
          ...
          throw new Error(
            `Pipeline processing failed: ${pipelineResponse.status} - ${errorText}`,
          );
        }
        ...
        dispatch(
          addProcessingLog({
            level: "SUCCESS",
            message:
              "Audio pipeline processing completed, sending to whisper service...",
            timestamp: Date.now(),
          }),
        );
        ...
        if (pipelineResult.processed_audio_url) {
          ...
          const processedAudioResponse = await fetch(
            pipelineResult.processed_audio_url,
          );
        ...
        return {
          ...
          processing_time_ms:
            (pipelineResult.total_processing_time_ms || 0) +
  ...
  const processTranscriptionForTranslation = useCallback(
    async (
  ...
  const processAudioWithTranslation = useCallback(
    async (
      audioBlob?: Blob,
      targetLanguages: string[] = [...DEFAULT_TARGET_LANGUAGES],
    ) => {
      ...
      try {
        ...
        const transcriptionText =
          transcriptionResult.transcription_result?.text ||
          transcriptionResult.transcription_result?.transcription ||
          "";

        if (!transcriptionText) {
          throw new Error(
            "No transcription text received from whisper service",
          );
        }

        const sourceLanguage =
          transcriptionResult.transcription_result?.language || "auto";
        ...
        return {
          ...
          total_processing_time:
            transcriptionResult.processing_time_ms +
            (translationResult.processing_time || 0),
        };
      } catch (error) {
        dispatch(
          addProcessingLog({
