...
export const useApiClient = (config?: Partial<ApiClientConfig>) => {
    ...
    async <T>(
      ...
      try {
        ...
        if (
          ...
          console.log(
            `API request failed, retrying... (${retryCount + 1}/${defaultConfig.retries})`,
          );
  ...
  const spawnBot = useCallback(
    async (request: BotSpawnRequest) => {
      const response = await apiRequest<{ botId: string; status: string }>(
        "/bot/spawn",
        {
          method: "POST",
      ...
      if (response.success) {
        ...
        notifyError(
          "Bot Spawn Failed",
          response.error || "Failed to spawn bot",
        );
      }
  ...
  const uploadAudio = useCallback(
    async (audioBlob: Blob, options: any = {}) => {
  ...
  const sendMessage = useCallback(
    async <T extends WebSocketEventType>(
      type: T,
      ...
      switch (type) {
        ...
        case "audio:chunk":
          if ("chunk" in data) {
            const audioBlob = new Blob([data.chunk as ArrayBuffer], {
              type: "audio/webm",
            });
        ...
        default:
          ...
          notifyWarning(
            "Feature Unavailable",
            `${type} requires WebSocket connection`,
          );
