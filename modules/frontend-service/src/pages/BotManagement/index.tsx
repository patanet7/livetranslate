...
const BotManagement: React.FC = () => {
  const { bots, activeBotIds, systemStats } = useAppSelector(
    (state) => state.bot,
  );
  ...
  const { terminateBot, getSystemStats, refreshBots, error } = useBotManager();
      ...
      <Box sx={{ mb: 4 }}>
        ...
        >
          <div>
            ...
            <Typography variant="body1" color="text.secondary">
              Google Meet bot lifecycle management, virtual webcam control, and
        ...
        <Paper sx={{ width: "100%", mb: 3 }}>
          ...
          >
            <Tab
              icon={
                <Badge badgeContent={activeBotCount} color="primary">
                  <BotIcon />
                </Badge>
              }
              label="Active Bots"
            />
            <Tab icon={<VideocamIcon />} label="Virtual Webcam" />
            <Tab icon={<StorageIcon />} label="Session Database" />
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} lg={4}>
            <BotSpawner
              onBotSpawned={(botId) => {
                showNotification(
                  `Bot ${botId} spawned successfully`,
                  "success",
                );
                refreshBots();
              }}
