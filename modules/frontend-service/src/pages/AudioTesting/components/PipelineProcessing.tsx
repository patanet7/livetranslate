...
const pipelineStages = [
  {
    ...
    icon: '🎤',
    metrics: ['Input Level (dB)', 'Sample Rate', 'Duration (s)', 'Format', 'Clipping Detection']
  },
  {
    id: 'decoded_audio',
    name: 'Audio Decoding',
    ...
    icon: '🔧',
    metrics: ['Decode Time (ms)', 'Memory Usage', 'Channel Count', 'Bit Depth', 'Compression Ratio']
  },
  {
    ...
    icon: '🎵',
    metrics: ['Filter Range (Hz)', 'Voice Band Gain', 'Frequency Response', 'Formant Preservation', 'Sibilance Enhancement']
  },
  {
    ...
    icon: '🗣️',
    metrics: ['Speech Segments', 'Voice Activity Ratio', 'Silence Removed (ms)', 'VAD Confidence', 'Energy Threshold']
  },
  {
    ...
    icon: '🔇',
    metrics: ['Noise Reduction (dB)', 'Speech Preservation', 'SNR Improvement', 'Artifact Detection', 'Voice Protection']
  },
  {
    ...
    icon: '✨',
    metrics: ['Clarity Score', 'Compression Ratio', 'Gain Applied (dB)', 'Presence Boost', 'Dynamic Range']
  },
  {
    ...
    icon: '🎯',
    metrics: ['De-essing (dB)', 'EQ Adjustment', 'Warmth Factor', 'Articulation', 'Meeting Optimization']
  },
  {
    ...
    icon: '✂️',
    metrics: ['Trimmed Duration (ms)', 'Pause Preservation', 'Natural Rhythm', 'Speaking Rate', 'Silence Threshold']
  },
  {
    ...
    icon: '🔄',
    metrics: ['Target Sample Rate', 'Resampling Quality', 'Anti-aliasing', 'Phase Coherence', 'Frequency Response']
  },
  {
    id: 'final_output',
    name: 'Final Output',
    ...
    icon: '📤',
    metrics: ['Final Level (dB)', 'Quality Score', 'Processing Time (ms)', 'File Size', 'Meeting Readiness']
  }
];
        ...
        <CardContent sx={{ pt: 0 }}>
            ...
            <>
              ...
              <Paper sx={{ p: 1.5, mb: 2, backgroundColor: 'grey.50' }}>
                <Grid container spacing={1}>
                  ...
                  {stageState.result.artifacts_detected !== undefined && (
                    <Grid item xs={6}>
                      ...
                      <Typography variant="caption" fontWeight="bold" sx={{ ml: 1 }}>
                        {stageState.result.artifacts_detected ? '⚠️ Detected' : '✅ None'}
                      </Typography>
                    </Grid>
                  )}
                </Grid>
                ...
                {stageState.result.stage_specific_data && (
                  <Box sx={{ mt: 1 }}>
                    <Typography variant="caption" fontWeight="bold">Stage Data:</Typography>
                    <Grid container spacing={1} sx={{ mt: 0.5 }}>
                      {Object.entries(stageState.result.stage_specific_data).map(([key, value]) => (
          ...
          {stageState?.error && (
            <>
              ...
              <Paper sx={{ p: 1.5, backgroundColor: 'error.light', color: 'error.contrastText' }}>
  ...
  return (
    <Box>
        ...
        {!hasRecording && (
          <Box sx={{ p: 2, backgroundColor: 'warning.light', borderRadius: 1, mb: 2 }}>
            <Typography variant="body2" color="warning.contrastText">
              ⚠️ No audio available for processing. Please record audio or upload a file first.
            </Typography>
          </Box>
        )}
      </Paper>
            ...
            <Grid item xs={12} md={3}>
              <Box sx={{ textAlign: 'center' }}>
                <Typography variant="h4" color="success.main">
                  {stages.reduce((total, stage) => total + (stage.processingTime || 0), 0).toFixed(0)}ms
                </Typography>
            ...
            <Grid item xs={12} md={3}>
              <Box sx={{ textAlign: 'center' }}>
                <Typography variant="h4" color="info.main">
                  {stages.length > 0 ? 
                    ((stages.filter(s => s.status === 'completed').length / stages.length) * 100).toFixed(0) + '%'
                    : '0%'
                  }
                </Typography>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Each stage processes audio with specific optimizations for meeting voice isolation and clarity enhancement.
      </Typography>
      <Grid container spacing={2}>
        {pipelineStages.map((stage, index) => {
          const stageState = stages.find(s => s.id === stage.id);
          const isActive = isProcessing && Math.floor(progress * pipelineStages.length) === index;
