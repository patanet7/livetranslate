...
const AudioTesting: React.FC = () => {
  ...
  const { recording, visualization, stages, config } = useAppSelector(
    (state) => state.audio,
  );
  ...
  const handleTabChange = useCallback(
    (__event: React.SyntheticEvent, newValue: number) => {
  ...
  const handleProcessTranscriptionForTranslation = useCallback(async () => {
    if (!transcriptionResult?.text && !transcriptionResult?.transcription) {
      dispatch(
        addProcessingLog({
          level: "ERROR",
          message:
            "No transcription available. Please run transcription first.",
          timestamp: Date.now(),
        }),
      );
      return;
    ...
    try {
      ...
      const result = await processTranscriptionForTranslation(
        transcriptionText,
        sourceLanguage,
        targetLanguages,
      );
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
        Professional audio testing suite with real-time processing pipeline
        control and comprehensive analysis tools
      </Typography>

      <Alert severity="info" sx={{ mb: 3 }}>
        <Typography variant="body2">
          This audio testing interface provides complete control over the audio
          processing pipeline. Configure recording settings, monitor real-time
          audio, and test the full processing workflow.
        </Typography>
      </Alert>
                ...
                <CardContent>
                  ...
                  <Stack spacing={2}>
                    ...
                    >
                      {isProcessing
                        ? "Processing..."
                        : "🚀 Complete Pipeline (Audio → Translation)"}
                    </Button>
                  </Stack>
                </CardContent>
              </Card>
              ...
              <Card sx={{ mb: 3 }}>
                <CardContent>
                  ...
                  {transcriptionResult ? (
                    ...
                    <Alert severity="info">
                      No transcription available. Record audio and process with
                      translation to see results.
                    </Alert>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent>
                  ...
                  Object.keys(translationResults).length > 0 ? (
                    <Box>
                      {Object.entries(translationResults).map(
                          ...
                          <Box key={lang} sx={{ mb: 2 }}>
                            ...
                            {translation.confidence && (
                              ...
                              >
                                ...
                                Time:{" "}
                                {translation.processing_time?.toFixed(2) ||
                                  "N/A"}
                                ms
                              </Typography>
                            )}
                    ...
                    <Alert severity="info">
                      No translations available. Select target languages and
                      process audio to see translation results.
                    </Alert>
                  )}
                </CardContent>
              </Card>
