...
const AUDIO_PROCESSING_TABS: AudioProcessingTab[] = [
  {
    ...
    icon: <AnalyticsIcon />,
    description:
      "Real-time system monitoring, performance metrics, and service health",
  ...
  },
  {
    ...
    icon: <PipelineIcon />,
    description:
      "Visual audio pipeline editor with drag-and-drop processing stages",
  ...
  },
  {
    ...
    description:
      "Advanced transcription testing with multiple models and languages",
...
const AudioProcessingHub: React.FC<AudioProcessingHubProps> = ({
  ...
  const isConnected = useAppSelector(
    (state) => state.websocket.connection.isConnected,
  );
  ...
  const getCurrentTabConfig = () => {
    return (
      AUDIO_PROCESSING_TABS.find((tab) => tab.id === currentTab) ||
      AUDIO_PROCESSING_TABS[0]
    );
  };
    ...
    >
      ...
      <Box sx={{ mb: 3, px: 1.5 }}>
        ...
        {!isConnected && (
          ...
          >
            WebSocket connection lost. Some real-time features may be
            unavailable.
          </Alert>
        )}
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          <Typography variant="body2" color="textSecondary">
            Services: {systemHealth ? Object.keys(systemHealth).length : 0}{" "}
            active
          </Typography>
          <Typography variant="body2" color="textSecondary">
