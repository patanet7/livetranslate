...
describe("BotManagement Integration", () => {
  ...
  describe("full bot lifecycle workflow", () => {
    it("should complete the full bot management workflow", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Bot Management Dashboard"),
        ).toBeInTheDocument();
      });
      ...
      expect(
        screen.getByText(mockBot.meetingInfo.meetingTitle!),
      ).toBeInTheDocument();
      ...
      await waitFor(() => {
        expect(mockFetch).toHaveBeenCalledWith(
          `/api/bot/${mockBot.botId}/terminate`,
          {
            method: "POST",
          },
        );
  ...
  describe("tab navigation and functionality", () => {
    it("should navigate between tabs and show appropriate content", async () => {
      ...
      mockFetch
        ...
        .mockResolvedValueOnce(
          mockApiResponse(createMockSystemStats()) as Promise<Response>,
        );
      ...
      expect(
        screen.getByText("Bot Configuration Settings"),
      ).toBeInTheDocument();
    });
  });
