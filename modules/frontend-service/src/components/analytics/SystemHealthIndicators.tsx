...
const SystemHealthIndicators: React.FC<SystemHealthIndicatorsProps> = ({
  ...
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(
    new Set(["service", "infrastructure"]),
  );
    ...
    if (analyticsData.systemMetrics) {
      ...
      if (metrics.cpu) {
        realIndicators.push({
          ...
          status:
            metrics.cpu.percentage > 90
              ? "critical"
              : metrics.cpu.percentage > 70
                ? "warning"
                : "healthy",
      ...
      if (metrics.performance?.queueLength !== undefined) {
        realIndicators.push({
          ...
          category: "performance",
          status:
            metrics.performance.queueLength > 100
              ? "critical"
              : metrics.performance.queueLength > 50
                ? "warning"
                : "healthy",
  ...
  const loadHealthData = useCallback(async () => {
    ...
    try {
      ...
      const { indicators: newIndicators, services: newServices } =
        await transformHealthData();
      ...
      <Box sx={{ p: 3 }}>
        <Alert severity="error" sx={{ mb: 2 }}>
          ...
          <Typography variant="body2">
            Cannot retrieve system health indicators. Please ensure the backend
    ...
    <Box>
      ...
      {hasError && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          <Typography variant="body2">
            API Connection Lost - Displaying cached data from{" "}
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" color="textSecondary">
                {healthyCount} healthy, {warningCount} warnings, {criticalCount}{" "}
                critical
              </Typography>
            </Box>
          </Box>
      ...
      >
        ...
        <Typography variant="caption" color="textSecondary">
          Auto-refresh:{" "}
          {autoRefresh ? `${refreshInterval / 1000}s` : "disabled"}
        </Typography>
      </Box>
    </Box>
  );
