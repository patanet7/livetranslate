...
const PerformanceCharts: React.FC<PerformanceChartsProps> = ({
  ...
  const [selectedChart, setSelectedChart] = useState<'overview' | 'latency' | 'throughput' | 'errors' | 'resources'>('overview');
  ...
  const chartConfigs: Record<string, ChartConfig> = {
    overview: {
      ...
      metrics: [
        { name: 'CPU Usage', color: theme.palette.primary.main, unit: '%', dataKey: 'cpu' },
        { name: 'Memory Usage', color: theme.palette.secondary.main, unit: '%', dataKey: 'memory' },
        { name: 'Request Rate', color: theme.palette.success.main, unit: 'req/s', dataKey: 'requests' },
      ],
      timeRange,
    ...
    latency: {
      ...
      metrics: [
        { name: 'Avg Response Time', color: theme.palette.primary.main, unit: 'ms', dataKey: 'avgResponseTime' },
        { name: 'P95 Response Time', color: theme.palette.warning.main, unit: 'ms', dataKey: 'p95ResponseTime' },
        { name: 'P99 Response Time', color: theme.palette.error.main, unit: 'ms', dataKey: 'p99ResponseTime' },
      ],
      timeRange,
    ...
    throughput: {
      ...
      metrics: [
        { name: 'Audio Requests', color: theme.palette.primary.main, unit: 'req/s', dataKey: 'audioRequests' },
        { name: 'Translation Requests', color: theme.palette.secondary.main, unit: 'req/s', dataKey: 'translationRequests' },
        { name: 'Pipeline Requests', color: theme.palette.success.main, unit: 'req/s', dataKey: 'pipelineRequests' },
      ],
      timeRange,
    ...
    errors: {
      ...
      metrics: [
        { name: 'HTTP 4xx', color: theme.palette.warning.main, unit: 'errors/min', dataKey: 'http4xx' },
        { name: 'HTTP 5xx', color: theme.palette.error.main, unit: 'errors/min', dataKey: 'http5xx' },
        { name: 'Timeouts', color: theme.palette.info.main, unit: 'errors/min', dataKey: 'timeouts' },
      ],
      timeRange,
    ...
    resources: {
      ...
      metrics: [
        { name: 'CPU Cores', color: theme.palette.primary.main, unit: 'cores', dataKey: 'cpuCores' },
        { name: 'Memory GB', color: theme.palette.secondary.main, unit: 'GB', dataKey: 'memoryGB' },
        { name: 'Disk I/O', color: theme.palette.success.main, unit: 'MB/s', dataKey: 'diskIO' },
      ],
      timeRange,
      refreshInterval: 5000,
    },
  ...
  const generateMockData = useCallback((points: number = 50) => {
    const now = new Date();
    const interval = timeRange === '1h' ? 60000 : 
                   timeRange === '6h' ? 360000 :
                   timeRange === '24h' ? 1440000 :
                   timeRange === '7d' ? 10080000 : 43200000;
    ...
    for (let i = points - 1; i >= 0; i--) {
      ...
      data.push({
        timestamp,
        // System metrics
        cpu: Math.max(10, Math.min(95, baseLoad * 60 * spike + Math.random() * 10)),
        memory: Math.max(20, Math.min(90, baseLoad * 70 + Math.random() * 15)),
        requests: Math.max(0, baseLoad * 100 * spike + Math.random() * 20),
        
        // Latency metrics (inversely correlated with load)
        avgResponseTime: Math.max(50, (1 + baseLoad) * 150 * spike + Math.random() * 50),
        p95ResponseTime: Math.max(100, (1 + baseLoad) * 300 * spike + Math.random() * 100),
        p99ResponseTime: Math.max(200, (1 + baseLoad) * 600 * spike + Math.random() * 200),
  ...
  return (
    <Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Tooltip title="Refresh Data">
                <IconButton onClick={loadData} size="small" disabled={isLoading}>
                  <Refresh />
                </IconButton>
              </Tooltip>
      ...
      <Grid container spacing={2}>
        {currentConfig.metrics.map((metric) => {
          const values = data.map(d => d[metric.dataKey]).filter(v => typeof v === 'number');
