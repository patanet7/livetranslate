...
const FFTSpectralAnalyzer: React.FC<FFTSpectralAnalyzerProps> = ({
  ...
  const [fundamentalFrequency, setFundamentalFrequency] = useState<
    number | null
  >(null);
  ...
  const generateMockFFTData = useCallback((): FFTData[] => {
    ...
    for (let i = 0; i < bins; i++) {
      ...
      if (
        frequency < settings.frequencyRange[0] ||
        frequency > settings.frequencyRange[1]
      ) {
        continue;
      }
      ...
      fundamentals.forEach((fund) => {
        ...
        if (distance < peakWidth) {
          const peakGain =
            40 *
            Math.exp(
              -(distance * distance) / (2 * (peakWidth / 3) * (peakWidth / 3)),
            );
          magnitude += peakGain;
        }
      });
      ...
      magnitude +=
        5 * Math.sin(Date.now() / 1000 + frequency / 1000) * Math.random();

      data.push({
        frequency,
        magnitude: Math.max(
          settings.magnitudeRange[0],
          Math.min(settings.magnitudeRange[1], magnitude),
        ),
      ...
      for (let i = 1; i < data.length - 1; i++) {
        ...
        if (
          ...
          if (!tooClose) {
            ...
            while (leftEdge > 0 && data[leftEdge].magnitude > threshold)
              leftEdge--;
            while (
              rightEdge < data.length - 1 &&
              data[rightEdge].magnitude > threshold
            )
              rightEdge++;

            const bandwidth =
              data[rightEdge].frequency - data[leftEdge].frequency;
  ...
  const analyzeHarmonics = useCallback(
    (peaks: SpectralPeak[]): void => {
      ...
      const fundamental = sortedByFreq.find(
        (peak) => peak.frequency > 80 && peak.frequency < 2000,
      );
              ...
              >
                <Alert severity="info">
                  {isAnalyzing
                    ...
                    : "Click play to start spectral analysis"}
                </Alert>
              </Box>
            )}
          </Box>
            ...
            >
              <CardContent>
                ...
                <Typography variant="body2" color="textSecondary">
                  ...
                  {fundamentalFrequency > 400 && fundamentalFrequency < 480
                    ? "A4"
                    : "Unknown"}
                </Typography>
              </CardContent>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} sm={3}>
                  ...
                  <Typography variant="h6">
                    {(48000 / settings.fftSize).toFixed(1)} Hz
                  </Typography>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Typography variant="body2" color="textSecondary">
