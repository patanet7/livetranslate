...
import { CloudUpload, AudioFile, PlayArrow, Info } from "@mui/icons-material";
...
const STAGE_INFO: Record<string, StageInfo> = {
  vad: {
    description:
      "Voice Activity Detection - Identifies speech segments and filters silence",
    processingTime: "< 5ms",
  ...
  voice_filter: {
    description:
      "Voice Frequency Filtering - Enhances speech frequencies while preserving natural voice",
    processingTime: "< 8ms",
  ...
  noise_reduction: {
    description:
      "Spectral Noise Reduction - Removes background noise using frequency domain analysis",
    processingTime: "< 15ms",
  ...
  voice_enhancement: {
    description:
      "Voice Enhancement - Improves speech clarity, presence, and intelligibility",
    processingTime: "< 10ms",
  ...
  equalizer: {
    description:
      "Parametric Equalizer - Multi-band frequency response shaping with professional presets",
    processingTime: "< 12ms",
  ...
  spectral_denoising: {
    description:
      "Advanced Spectral Denoising - FFT-based noise reduction with multiple algorithms",
    processingTime: "< 20ms",
  ...
  conventional_denoising: {
    description:
      "Conventional Denoising - Time-domain filtering with traditional methods",
    processingTime: "< 8ms",
  ...
  lufs_normalization: {
    description:
      "LUFS Normalization - Professional loudness normalization (ITU-R BS.1770-4)",
    processingTime: "< 18ms",
  ...
  agc: {
    description:
      "Auto Gain Control - Adaptive level control with lookahead processing",
    processingTime: "< 12ms",
  ...
  compression: {
    description:
      "Dynamic Range Compression - Professional dynamics control for consistent levels",
    processingTime: "< 8ms",
  ...
  limiter: {
    description:
      "Peak Limiting - Transparent brick-wall limiting to prevent clipping",
    processingTime: "< 6ms",
    ...
    (acceptedFiles: File[]) => {
      ...
      if (acceptedFiles.length > 0) {
        ...
        if (!supportedFormats.includes(file.type)) {
          setError(
            `Unsupported file format. Supported: ${supportedFormats.map((f) => f.split("/")[1]).join(", ")}`,
          );
          return;
        }
    ...
    <Card sx={{ height: "100%" }}>
      <CardContent>
        ...
        >
          {isProcessing
            ? "Processing..."
            : `Process through ${stageDisplayName}`}
        </Button>
      </CardContent>
    </Card>
  );
