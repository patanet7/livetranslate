...
const AUDIO_COMPONENT_LIBRARY: AudioComponent[] = [
  ...
  },
  {
    ...
    description:
      "Raw microphone capture (browser preprocessing disabled - use pipeline stages for effects)",
    icon: Mic,
    category: "Input Sources",
    complexity: "basic",
    processingTime: { target: 0, max: 0 },
  ...
  },
  {
    ...
    parameters: [
      ...
      },
      {
        ...
        step: 0.1,
        description:
          "Adjust voice brightness (negative=darker, positive=brighter)",
      },
    ],
    tags: [
      "enhancement",
...
const ComponentLibrary: React.FC<ComponentLibraryProps> = ({
  ...
  const filteredComponents = AUDIO_COMPONENT_LIBRARY.filter((component) => {
    ...
    const matchesType =
      filterByType === "all" || component.type === filterByType;
  ...
  return (
    <Card sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        >
          <Tab
            label={`All (${AUDIO_COMPONENT_LIBRARY.length})`}
            sx={{ minHeight: 36, py: 1 }}
          />
          <Tab
            label={`Input (${AUDIO_COMPONENT_LIBRARY.filter((c) => c.type === "input").length})`}
            sx={{ minHeight: 36, py: 1 }}
          />
          <Tab
            label={`Processing (${AUDIO_COMPONENT_LIBRARY.filter((c) => c.type === "processing").length})`}
            sx={{ minHeight: 36, py: 1 }}
          />
          <Tab
            label={`Output (${AUDIO_COMPONENT_LIBRARY.filter((c) => c.type === "output").length})`}
            sx={{ minHeight: 36, py: 1 }}
          />
        </Tabs>
          ...
          {Object.entries(componentsByCategory).map(
            ([category, components]) => (
              <Box key={category} mb={2}>
                                ...
                                <Chip
                                  ...
                                  sx={{
                                    ...
                                    backgroundColor: getProcessingTimeColor(
                                      component.processingTime.target,
                                    ),
                                    color: "white",
                                  }}
                                />
                ...
                {Object.keys(componentsByCategory).indexOf(category) <
                  Object.keys(componentsByCategory).length - 1 && (
                  <Divider sx={{ mt: 1 }} />
                )}
              </Box>
            ),
        ...
        <Box mt={1} p={1} bgcolor="action.hover" borderRadius={1}>
          <Typography variant="caption" color="text.secondary">
            ...
            {AUDIO_COMPONENT_LIBRARY.length} components
          </Typography>
        </Box>
      </CardContent>
    </Card>
