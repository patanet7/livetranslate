...
export const SpectralAnalyzer: React.FC<SpectralAnalyzerProps> = ({
  ...
  const [spectralDisplay, setSpectralDisplay] =
    useState<SpectralDisplay>("log_power");
  ...
  const [smoothedSpectrum, setSmoothedSpectrum] = useState<Float32Array | null>(
    null,
  );
  ...
  const performAdvancedSpectralAnalysis = useCallback(() => {
    ...
    if (fundamental > 0) {
      ...
      harmonicDistortion =
        harmonicDistortion /
        smoothedSpectrum[Math.round(fundamental / binWidth)];
    }
    ...
    spectralSpread =
      totalMagnitude > 0 ? Math.sqrt(spectralSpread / totalMagnitude) : 0;
    ...
    if (spectralHistory.length > 0) {
      ...
      for (
        let i = 0;
        i < Math.min(smoothedSpectrum.length, prevSpectrum.length);
        i++
      ) {
    ...
    const voiceProbability = calculateVoiceProbability(
      smoothedSpectrum,
      binWidth,
      fundamental,
    );
    const pitchConfidence =
      fundamental > 0
        ? Math.min(
            1.0,
            smoothedSpectrum[Math.round(fundamental / binWidth)] * 2,
          )
        : 0;
    ...
    const analysis: SpectralAnalysis = {
      ...
      spectral_features: {
        ...
        flux: spectralFlux,
        kurtosis: calculateSpectralKurtosis(
          smoothedSpectrum,
          spectralCentroid,
          spectralSpread,
        ),
        skewness: calculateSpectralSkewness(
          smoothedSpectrum,
          spectralCentroid,
          spectralSpread,
        ),
  ...
  const drawSpectralAnalysis = useCallback(() => {
    ...
    const endBin = Math.min(
      Math.floor(frequencyRange[1] / binWidth),
      fft_data.length / 2,
    );
    ...
    for (let i = startBin; i < endBin; i++) {
      ...
      switch (spectralDisplay) {
        ...
        case "power":
          ...
          break;
        case "log_power":
          processedData[i - startBin] = Math.log10(
            Math.max(value * value, 1e-10),
          );
          break;
        case "mel_scale":
    ...
    const minValue =
      spectralDisplay === "log_power" ? Math.min(...processedData) : 0;
  ...
  const drawFormantView = (
    ...
    if (currentAnalysis && showFormants) {
      ...
      [formant_analysis.f1, formant_analysis.f2, formant_analysis.f3].forEach(
        (formant, index) => {
          if (formant > frequencyRange[0] && formant < frequencyRange[1]) {
            const x =
              ((formant - frequencyRange[0]) /
                (frequencyRange[1] - frequencyRange[0])) *
              width;
  ...
  const drawVoiceView = (
    ...
    voiceRegions.forEach((region) => {
      const startX = Math.max(
        0,
        ((region.min - frequencyRange[0]) /
          (frequencyRange[1] - frequencyRange[0])) *
          width,
      );
      const endX = Math.min(
        width,
        ((region.max - frequencyRange[0]) /
          (frequencyRange[1] - frequencyRange[0])) *
          width,
      );
  ...
  const drawNoiseView = (
    ...
    if (currentAnalysis) {
      ...
      const noiseY =
        height -
        ((Math.log10(Math.pow(10, noise_analysis.noise_floor / 10)) -
          minValue) /
          range) *
          height *
          0.9;
      ...
      ctx.font = "12px monospace";
      ctx.fillText(
        `Noise Floor: ${noise_analysis.noise_floor.toFixed(1)} dB`,
        10,
        noiseY - 5,
      );
    }
  ...
  const drawFrequencyLabels = (
    ...
    for (let i = 0; i <= labelCount; i++) {
      ...
      const label =
        freq < 1000 ? `${Math.round(freq)}` : `${(freq / 1000).toFixed(1)}k`;
  ...
  const estimateFormants = (spectrum: Float32Array, binWidth: number) => {
    ...
    for (
      let i = voiceStart + 2;
      i < Math.min(voiceEnd - 2, spectrum.length / 2);
      i++
    ) {
      if (
  ...
  const calculateVoiceProbability = (
    ...
    const pitchStrength =
      fundamental > 0
        ? Math.min(1, spectrum[Math.round(fundamental / binWidth)])
        : 0;
  ...
  const calculateSpectralKurtosis = (
    _spectrum: Float32Array,
    _centroid: number,
    _spread: number,
  ): number => {
  ...
  const calculateSpectralSkewness = (
    _spectrum: Float32Array,
    _centroid: number,
    _spread: number,
  ): number => {
    ...
    return (
      <Card sx={{ height: "100%" }}>
        <CardContent>
          ...
          <Alert severity="info">
            No audio data available for spectral analysis. Start audio
            processing to see advanced frequency domain analysis.
          </Alert>
        </CardContent>
      </Card>
    );
      ...
      <CardContent>
            ...
            <Grid item xs={6} md={2}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  label="Display"
                  onChange={(e) =>
                    setSpectralDisplay(e.target.value as SpectralDisplay)
                  }
                >
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="caption" display="block">
                    Centroid:{" "}
                    {currentAnalysis.spectral_features.centroid.toFixed(1)} Hz
                  </Typography>
                  <Typography variant="caption" display="block">
                    Rolloff:{" "}
                    {currentAnalysis.spectral_features.rolloff.toFixed(1)} Hz
                  </Typography>
                  <Typography variant="caption" display="block">
                    Bandwidth:{" "}
                    {currentAnalysis.spectral_features.bandwidth.toFixed(1)} Hz
                  </Typography>
                  <Typography variant="caption" display="block">
                    Flatness:{" "}
                    {currentAnalysis.spectral_features.flatness.toFixed(3)}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="caption" display="block">
                    Voice Probability:{" "}
                    {(
                      currentAnalysis.voice_characteristics.voice_probability *
                      100
                    ).toFixed(1)}
                    %
                  </Typography>
                  <Typography variant="caption" display="block">
                    Pitch Confidence:{" "}
                    {(
                      currentAnalysis.voice_characteristics.pitch_confidence *
                      100
                    ).toFixed(1)}
                    %
                  </Typography>
                  <Typography variant="caption" display="block">
                    V/UV Ratio:{" "}
                    {(
                      currentAnalysis.voice_characteristics
                        .voiced_unvoiced_ratio * 100
                    ).toFixed(1)}
                    %
                  </Typography>
                </Grid>
                ...
                {currentAnalysis.harmonic_content.fundamental > 0 && (
                  <Grid item xs={12} md={6}>
                    ...
                    <Typography variant="caption" display="block">
                      Fundamental:{" "}
                      {currentAnalysis.harmonic_content.fundamental.toFixed(1)}{" "}
                      Hz
                    </Typography>
                    <Typography variant="caption" display="block">
                      Harmonics:{" "}
                      {currentAnalysis.harmonic_content.harmonics.length}
                    </Typography>
                    <Typography variant="caption" display="block">
                      THD:{" "}
                      {(
                        currentAnalysis.harmonic_content.harmonic_distortion *
                        100
                      ).toFixed(2)}
                      %
                    </Typography>
                ...
                <Grid item xs={12} md={6}>
                  <Typography variant="body2" gutterBottom>
                    <strong>Noise Analysis</strong>
                  </Typography>
                  <Typography variant="caption" display="block">
                    SNR:{" "}
                    {currentAnalysis.noise_analysis.snr_estimate.toFixed(1)} dB
                  </Typography>
                  <Typography variant="caption" display="block">
                    Noise Floor:{" "}
                    {currentAnalysis.noise_analysis.noise_floor.toFixed(1)} dB
                  </Typography>
                  <Typography variant="caption" display="block">
                    Spectral Noise:{" "}
                    {currentAnalysis.noise_analysis.spectral_noise_level.toFixed(
                      1,
                    )}{" "}
                    dB
                  </Typography>
        ...
        <Box mt={2} textAlign="center">
          <Typography variant="caption" color="text.secondary">
            View: {analysisView} | Display: {spectralDisplay} | Range:{" "}
            {frequencyRange[0]}-{frequencyRange[1]} Hz | Resolution:{" "}
            {audioData.metadata
              ? (
                  audioData.metadata.sample_rate /
                  audioData.metadata.window_size
                ).toFixed(1)
              : 0}
            Hz/bin
          </Typography>
