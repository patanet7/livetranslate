...
export const LUFSMeter: React.FC<LUFSMeterProps> = ({
  ...
  const [measurementType, setMeasurementType] = useState<MeasurementType>('integrated');
  const [broadcastStandard, setBroadcastStandard] = useState<BroadcastStandard>('ebu_r128');
  ...
  const drawLUFSMeter = useCallback(() => {
    ...
    for (let lufs = lufsMin; lufs <= lufsMax; lufs += 6) {
      const y = meterY + meterHeight - ((lufs - lufsMin) / lufsRange) * meterHeight;
    ...
    const targetY = meterY + meterHeight - ((currentTarget - lufsMin) / lufsRange) * meterHeight;
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`${currentTarget} LUFS`, meterX + meterWidth + 15, targetY + 3);
    
    // Draw warning zones
    const warningZones = [
      { min: -9, max: 0, color: 'rgba(255, 0, 0, 0.2)' }, // Red zone (too loud)
      { min: -18, max: -9, color: 'rgba(255, 193, 7, 0.2)' }, // Yellow zone (caution)
      { min: currentTarget - 3, max: currentTarget + 3, color: 'rgba(0, 255, 136, 0.2)' } // Green zone (target)
    ];
    
    warningZones.forEach(zone => {
      const zoneTop = meterY + meterHeight - ((zone.max - lufsMin) / lufsRange) * meterHeight;
      const zoneBottom = meterY + meterHeight - ((zone.min - lufsMin) / lufsRange) * meterHeight;
    ...
    const measurements = {
      ...
      momentary: lufs_measurements.momentary_lufs[lufs_measurements.momentary_lufs.length - 1] || -60,
      short_term: lufs_measurements.short_term_lufs[lufs_measurements.short_term_lufs.length - 1] || -60
    };
    ...
    Object.entries(measurements).forEach(([type, value], index) => {
      if (type === 'integrated' || (type === 'momentary' && showMomentary) || (type === 'short_term' && showShortTerm)) {
        const barX = meterX + (index * (meterWidth / 3));
        const barWidth = meterWidth / 3 - 2;
        const barHeight = Math.max(1, ((Math.max(value, lufsMin) - lufsMin) / lufsRange) * meterHeight);
        ...
        if (type === measurementType) {
          const peakY = meterY + meterHeight - ((peakHold - lufsMin) / lufsRange) * meterHeight;
    ...
    if (measurementHistory.length > 1) {
      ...
      const pointSpacing = graphWidth / Math.max(measurementHistory.length - 1, 1);
      ...
      ['integrated', 'momentary', 'short_term'].forEach(type => {
        if (type === 'integrated' || (type === 'momentary' && showMomentary) || (type === 'short_term' && showShortTerm)) {
          ...
          measurementHistory.forEach((measurement, index) => {
            ...
            const value = type === 'integrated' ? measurement.integrated_lufs :
                         type === 'momentary' ? measurement.momentary_lufs :
                         measurement.short_term_lufs;
            const y = graphY + graphHeight - ((Math.max(value, lufsMin) - lufsMin) / lufsRange) * graphHeight;
      ...
      const targetGraphY = graphY + graphHeight - ((currentTarget - lufsMin) / lufsRange) * graphHeight;
      ctx.strokeStyle = '#00ff88';
      ctx.lineWidth = 1;
      ctx.setLineDash([3, 3]);
      ctx.beginPath();
    ...
    ctx.fillText(`Integrated: ${measurements.integrated.toFixed(1)} LUFS`, meterX, valueY);
    
    if (showMomentary) {
      ctx.fillStyle = barColors.momentary;
      ctx.fillText(`Momentary: ${measurements.momentary.toFixed(1)} LUFS`, meterX, valueY + 15);
    }
    
    if (showShortTerm) {
      ctx.fillStyle = barColors.short_term;
      ctx.fillText(`Short-term: ${measurements.short_term.toFixed(1)} LUFS`, meterX, valueY + 30);
    }
    
    if (showTruePeak) {
      ctx.fillStyle = '#ffff00';
      ctx.fillText(`True Peak: ${lufs_measurements.true_peak_db.toFixed(1)} dBTP`, meterX, valueY + 45);
    }
    
  }, [audioData, currentTarget, showMomentary, showShortTerm, showTruePeak, measurementHistory, peakHold, measurementType]);
  ...
  const updateMeasurement = useCallback(() => {
    ...
    const measurement: LUFSMeasurement = {
      ...
      momentary_lufs: lufs_measurements.momentary_lufs[lufs_measurements.momentary_lufs.length - 1] || -60,
      short_term_lufs: lufs_measurements.short_term_lufs[lufs_measurements.short_term_lufs.length - 1] || -60,
  ...
  const handleDownloadReport = () => {
    ...
    const report = {
      ...
      compliance_analysis: {
        meets_target: Math.abs(currentMeasurement.integrated_lufs - currentTarget) <= 1,
        true_peak_safe: currentMeasurement.true_peak_db <= standard.max_true_peak,
        range_acceptable: currentMeasurement.lufs_range <= standard.max_range,
        overall_compliant: currentMeasurement.ebu_r128_compliant && currentMeasurement.broadcast_safe
      }
    };
  ...
  const getComplianceStatus = () => {
    ...
    if (currentMeasurement.ebu_r128_compliant && currentMeasurement.broadcast_safe) {
      return 'compliant';
    } else if (Math.abs(currentMeasurement.integrated_lufs - currentTarget) <= 3) {
      return 'warning';
    } else {
    ...
    return (
      <Card sx={{ height: '100%' }}>
        <CardContent>
          ...
          <Alert severity="info">
            No audio data available for LUFS measurement. Start audio processing to see broadcast-compliant loudness metering.
          </Alert>
        </CardContent>
      </Card>
    );
      ...
      <CardContent>
          ...
          <Grid item xs={12} md={3}>
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                onChange={(e) => setBroadcastStandard(e.target.value as BroadcastStandard)}
              >
        ...
        {showCompliance && currentMeasurement && (
          <Box mt={2} p={2} bgcolor="background.default" borderRadius={1}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6} md={3}>
                <Typography variant="caption" color="text.secondary">Integrated LUFS:</Typography>
                <Typography variant="body2">
                  {currentMeasurement.integrated_lufs.toFixed(1)} / {currentTarget} LUFS
                </Typography>
              </Grid>
              
              <Grid item xs={6} md={3}>
                <Typography variant="caption" color="text.secondary">Loudness Units:</Typography>
                <Typography variant="body2">
                  {currentMeasurement.loudness_units > 0 ? '+' : ''}{currentMeasurement.loudness_units.toFixed(1)} LU
                </Typography>
              </Grid>
              
              <Grid item xs={6} md={3}>
                <Typography variant="caption" color="text.secondary">True Peak:</Typography>
                <Typography variant="body2">
                  {currentMeasurement.true_peak_db.toFixed(1)} / {standard.max_true_peak} dBTP
                </Typography>
              </Grid>
              ...
              <Grid item xs={6} md={3}>
                ...
                <Typography variant="body2">
                  {currentMeasurement.lufs_range.toFixed(1)} / {standard.max_range} LU
                </Typography>
              </Grid>
            </Grid>
            ...
            {Math.abs(currentMeasurement.recommended_adjustment) > 0.5 && (
              <Alert severity="info" sx={{ mt: 1 }}>
                <Typography variant="body2">
                  Recommended adjustment: {currentMeasurement.recommended_adjustment > 0 ? '+' : ''}
                  {currentMeasurement.recommended_adjustment.toFixed(1)} dB to meet target
                </Typography>
              </Alert>
            )}
          </Box>
        ...
        <Box mt={2} textAlign="center">
          <Typography variant="caption" color="text.secondary">
            ...
            Target: {currentTarget} LUFS | 
            Max Peak: {standard.max_true_peak} dBTP | 
            Max Range: {standard.max_range} LU
          </Typography>
        </Box>
      </CardContent>
    </Card>
