...
export const FFTVisualizer: React.FC<FFTVisualizerProps> = ({
  ...
  const [windowFunction, setWindowFunction] =
    useState<WindowFunction>("hanning");
  ...
  const [currentAnalysis, setCurrentAnalysis] =
    useState<FrequencyAnalysis | null>(null);
  ...
  const drawFFTSpectrum = useCallback(() => {
    ...
    for (let i = 0; i < fftData.length; i++) {
      smoothedFFT[i] =
        smoothedFFT[i] * smoothing + fftData[i] * (1 - smoothing);
    }
    ...
    const minValue =
      displayMode === "log_magnitude" ? Math.min(...processedData) : 0;
    ...
    for (let i = 0; i < processedData.length; i++) {
      ...
      const normalizedValue =
        range > 0 ? (processedData[i] - minValue) / range : 0;
    ...
    if (showVoiceRegion) {
      const voiceStart = Math.max(
        0,
        ((85 - frequencyRange[0]) / (frequencyRange[1] - frequencyRange[0])) *
          width,
      );
      const voiceEnd = Math.min(
        width,
        ((300 - frequencyRange[0]) / (frequencyRange[1] - frequencyRange[0])) *
          width,
      );
    ...
    for (let i = 0; i <= labelCount; i++) {
      const freq =
        frequencyRange[0] +
        (i / labelCount) * (frequencyRange[1] - frequencyRange[0]);
    ...
    return (
      <Card sx={{ height: "100%" }}>
        <CardContent>
          ...
          <Alert severity="info">
            No audio data available for FFT analysis. Start audio processing to
            see real-time frequency spectrum.
          </Alert>
        </CardContent>
      </Card>
    );
    ...
    <Card sx={{ height: "100%" }}>
      <CardContent>
            ...
            <Grid item xs={6} md={3}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  label="Display Mode"
                  onChange={(e) =>
                    setDisplayMode(e.target.value as FFTDisplayMode)
                  }
                >
            ...
            <Grid item xs={6} md={3}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setWindowFunction(e.target.value as WindowFunction)
                  }
                >
        ...
        {currentAnalysis && (
          <Box mt={2} p={2} bgcolor="background.default" borderRadius={1}>
            ...
            <Box mt={1}>
              ...
              <Typography variant="body2">
                Low:{" "}
                {(currentAnalysis.energy_distribution.low * 100).toFixed(1)}% |
                Mid:{" "}
                {(currentAnalysis.energy_distribution.mid * 100).toFixed(1)}% |
                High:{" "}
                {(currentAnalysis.energy_distribution.high * 100).toFixed(1)}%
              </Typography>
            </Box>
          </Box>
        )}
        ...
        <Box mt={2} textAlign="center">
          <Typography variant="caption" color="text.secondary">
            Sample Rate: {audioData.sampleRate}Hz | Window:{" "}
            {audioData.windowSize} samples | Resolution:{" "}
            {(audioData.sampleRate / audioData.windowSize).toFixed(1)}Hz/bin
          </Typography>
        </Box>
      </CardContent>
    </Card>
