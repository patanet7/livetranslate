...
class PromptOptimizer:
    ...
    def analyze_prompt_performance(
        self, prompt_id: str, metrics: list[PromptPerformanceMetric]
    ) -> dict[str, Any]:
...
class PromptManager:
    ...
    def get_prompts_for_language_pair(
        ...
        for prompt in self.prompts.values():
            ...
            if (
                "*" in prompt.language_pairs
                or pair in prompt.language_pairs
                or f"{target_lang}-{source_lang}" in prompt.language_pairs
            ):
        ...
        with self.lock:
            ...
            if metric.prompt_id in self.prompts:
                ...
                recent_metrics = [
                    ...
                    if m.prompt_id == metric.prompt_id and m.timestamp > time.time() - 86400
                ][-100:]

                if recent_metrics:
                    metrics["avg_quality"] = sum(m.quality_score for m in recent_metrics) / len(
                        recent_metrics
                    )
                    metrics["avg_speed"] = sum(m.processing_time for m in recent_metrics) / len(
                        recent_metrics
                    )
                    metrics["avg_confidence"] = sum(
                        m.confidence_score for m in recent_metrics
                    ) / len(recent_metrics)
                    metrics["success_rate"] = sum(1 for m in recent_metrics if m.success) / len(
                        recent_metrics
                    )
        ...
        for prompt_id in prompt_ids:
            ...
            if recent_metrics:
                comparison_data[prompt_id] = {
                    ...
                    / len(recent_metrics),
                    "avg_speed": sum(m.processing_time for m in recent_metrics)
                    / len(recent_metrics),
                    "avg_confidence": sum(m.confidence_score for m in recent_metrics)
                    / len(recent_metrics),
    ...
    def _generate_comparison_recommendation(self, ranked_prompts: list[tuple[str, dict]]) -> str:
        ...
        _best_prompt_id, best_data = ranked_prompts[0]
        ...
        _second_prompt_id, second_data = ranked_prompts[1]
    ...
    def get_prompt_statistics(self) -> dict[str, Any]:
        ...
        total_usage = sum(
            p.performance_metrics.get("usage_count", 0) for p in self.prompts.values()
        )
        ...
        return {
            ...
            "recent_activity": {
                ...
                "unique_prompts_used": len({m.prompt_id for m in recent_metrics}),
            ...
            "last_updated": max(
                (p.metadata.get("updated_at", 0) for p in self.prompts.values()), default=0
            ),
        }
...
if __name__ == "__main__":
    ...
    # Build prompt
    built_prompt, system_msg = manager.build_prompt(
        "test_prompt",
        {"source_language": "English", "target_language": "Spanish", "text": "Hello world"},
    )
