...
class LocalTranslationService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self._client = await get_inference_client_async(backend=self.preferred_backend)

            if self._client:
                self._is_initialized = True
                logger.info(
                    f"Local translation service initialized with {self._client.backend_type.value}"
                )
                return True
            else:
    ...
    async def translate(
        self, text: str, target_language: str, source_language: str | None = None, **kwargs
    ...
    async def translate_stream(
        self, text: str, target_language: str, source_language: str | None = None, **kwargs
    ) -> AsyncGenerator[LocalTranslationResponse, None]:
        """
...
def create_local_translation_service(
    backend: str | None = None, temperature: float = 0.3, max_tokens: int = 1024
    ...
    return LocalTranslationService(
        preferred_backend=backend, temperature=temperature, max_tokens=max_tokens
    )
