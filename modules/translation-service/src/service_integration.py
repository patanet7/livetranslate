...
class ServiceIntegration:
    ...
    def check_service_health(self, service_name: str) -> Dict:
        ...
        try:
            response = requests.get(
                f"{self.services[service_name]['rest']}/health", 
                timeout=5
            )
            return {
    ...
    async def translate_text(self, text: str, source_lang: str = "auto", target_lang: str = "auto") -> Dict:
        ...
        try:
            ...
            response = requests.post(
                f"{self.services['translation']['rest']}/translate",
                json=data,
                timeout=30
            )
    ...
    async def process_whisper_transcription(self, transcription_data: Dict) -> Dict:
        ...
        try:
            ...
            logger.info(f"✅ Translation complete: {text[:30]} → {translation_result.get('translation', '')[:30]}")
    ...
    async def send_to_speaker_service(self, processed_data: Dict) -> bool:
        ...
        try:
            ...
            # Send to speaker service for post-processing
            response = requests.post(
                f"{self.services['speaker']['rest']}/process",
                json=speaker_data,
                timeout=10
            )
    ...
    async def run_integration_loop(self):
        ...
        for service, status in health_status["services"].items():
            if status["status"] == "healthy":
                ...
                logger.warning(f"  ❌ {service}: {status['status']} - {status.get('error', 'Unknown error')}")
...
def main():
    ...
    parser.add_argument("--translation-port", type=int, default=8010, help="Translation service port")
