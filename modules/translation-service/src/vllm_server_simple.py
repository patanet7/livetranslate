...
class SimpleVLLMTranslationServer:
    ...
    def translate_text(self, input_text):
        ...
        try:
            ...
            logger.error(f"Translation error: {e}")
            return {
                "translation": f"Translation error: {e!s}",
                "original": input_text,
                "source_language": "unknown",
                "target_language": "unknown",
                "confidence_score": 0.0,
            ...
            async for message in websocket:
                try:
                    ...
                    if "text" in data:
                        ...
                        result = await loop.run_in_executor(None, self.translate_text, input_text)
    ...
    async def serve(self):
        ...
        async with websockets.serve(
            self.handle_client, self.host, self.port + 1
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=8010, help="Port to bind to (WebSocket will use port+1)"
    )
    parser.add_argument("--model", default="Qwen/Qwen2.5-14B-Instruct-AWQ", help="Model name")

    args = parser.parse_args()

    server = SimpleVLLMTranslationServer(host=args.host, port=args.port, model_name=args.model)
