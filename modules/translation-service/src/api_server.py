...
try:
    ...
    # Logger not yet configured, use print for early warning
    print("WARNING: OpenAI-compatible translator not available")
...
internal_vllm_server: SimpleVLLMTranslationServer | None = None
...
def validate_local_model(model_path: str) -> tuple:
    ...
    try:
        ...
        return (
            True,
            f"Model directory is valid: {len(model_files)} model files, {len(tokenizer_files)} tokenizer files",
        )

    except Exception as e:
        return False, f"Error validating model: {e!s}"
...
async def initialize_internal_vllm() -> SimpleVLLMTranslationServer | None:
    ...
    try:
        ...
        default_model = os.getenv(
            "TRANSLATION_MODEL", current_config.get("model_name", "./models/Llama-3.1-8B-Instruct")
        )
        ...
        if default_model.startswith("./") or default_model.startswith("/"):
            ...
            if not is_valid:
                ...
                logger.error(
                    "💡 The model directory should contain: config.json, tokenizer files, and model weights"
                )
                return None
            else:
        ...
        current_config.update(
            {"model_name": default_model, "internal_port": vllm_port, "internal_host": vllm_host}
        )
        ...
        internal_vllm_server = SimpleVLLMTranslationServer(
            host=vllm_host, port=vllm_port, model_name=default_model
        )
...
async def initialize_openai_compatible_translators():
        ...
        try:
            ...
            if await ollama.initialize():
                ...
                if available_models:
                    ...
                    if not model_valid:
                        logger.warning(
                            f"⚠️ Configured model '{configured_model}' not found in Ollama"
                        )
    ...
    if os.getenv("OPENAI_ENABLE", "false").lower() == "true":
        try:
            openai = create_openai_translator(
                model=os.getenv("OPENAI_MODEL", "gpt-4o-mini"), api_key=os.getenv("OPENAI_API_KEY")
            )
        ...
        try:
            ...
            if await vllm_server.initialize():
                ...
                if available_models:
                    ...
                    if not model_valid:
                        logger.warning(
                            f"⚠️ Configured model '{configured_model}' not found in vLLM server"
                        )
                        logger.warning(f"💡 Available models: {', '.join(available_models)}")
    ...
    if openai_compatible_translators:
        logger.info(
            f"🎉 Initialized {len(openai_compatible_translators)} OpenAI-compatible translator(s): {list(openai_compatible_translators.keys())}"
        )
    else:
        logger.info(
            "[i] No OpenAI-compatible translators enabled (set environment variables to enable)"
        )
...
async def initialize_service():
    ...
    try:
        ...
        if get_llama_translator and (
            "llama" in default_model.lower() or "meta-llama" in default_model.lower()
        ):
        ...
        if (
            not llama_translator or not getattr(llama_translator, "is_ready", False)
        ) and get_nllb_translator:
            ...
            logger.info(
                "💡 NLLB models use encoder-decoder architecture which requires transformers"
            )
            ...
            if nllb_translator and nllb_translator.is_ready:
                ...
                try:
                    ...
                    raise RuntimeError(
                        "Translation service initialization failed: No working backends available"
                    ) from ollama_error
...
def health_check():
    ...
    has_backend = (
        ...
        or (llama_translator and llama_translator.is_ready)
        or (nllb_translator and nllb_translator.is_ready)
        or bool(openai_compatible_translators)
    )
    if not has_backend:
    ...
    if llama_translator and llama_translator.is_ready:
        ...
        active_backend = next(iter(openai_compatible_translators.keys()))
    ...
    return jsonify(
        {
            ...
            "available_backends": list(openai_compatible_translators.keys())
            if openai_compatible_translators
            else [],
            "timestamp": datetime.now().isoformat(),
            "version": "1.0.0",
...
async def get_device_info():
    ...
    try:
        ...
        if (backend == "vllm" and gpu_available) or (backend == "triton" and gpu_available):
            device = "gpu"
            device_type = "gpu"
            acceleration = "cuda"
        elif backend == "ollama":
...
async def get_configuration():
    ...
    try:
        ...
        runtime_config.update(
            {
                ...
                "internal_vllm_ready": internal_vllm_server.is_model_ready
                if internal_vllm_server
                else False,
                "active_sessions": len(active_sessions),
                "timestamp": datetime.now().isoformat(),
            }
        ...
        return jsonify({"status": "success", "configuration": runtime_config})
...
async def translate_text():
    ...
    try:
                    ...
                    return jsonify(
                        {
                            ...
                            "source_language": llama_result.get(
                    ...
                    return jsonify(
                        {
                            ...
                            "session_id": session_id,
                            "timestamp": nllb_result.get(
                                "timestamp", datetime.utcnow().isoformat()
                            ),
                            "model_used": nllb_result.get(
                                "model_used", current_config.get("model_name", "nllb")
                            ),
                        }
                    )
                else:
        ...
        openai_backend = None
        specific_model = (
            None  # For specifying a particular model within a backend (e.g., qwen3:4b for Ollama)
        )
        ...
        if openai_backend and openai_backend.is_ready:
            logger.info(
                f"Using OpenAI-compatible translator: {openai_backend.config.name}"
                    ...
                    return jsonify(
                        {
                            ...
                            "backend_used": result.get("metadata", {}).get(
                                "backend_used", openai_backend.config.name
                            ),
                            "session_id": session_id,
                            "timestamp": datetime.utcnow().isoformat(),
                            "model_used": result.get("metadata", {}).get(
                                "model_used", openai_backend.config.model
                            ),
                        }
                    )
                else:
                    logger.warning("OpenAI-compatible translation returned no result")
    ...
    return jsonify(
        {
            ...
            "fallback_mode": getattr(translation_service, "fallback_mode", False)
...
def api_available_models():
    ...
    for backend_name, translator in openai_compatible_translators.items():
        ...
        if backend_name == "ollama" and translator.is_ready:
            try:
                ...
                logger.info(
                    f"Found {len(available_ollama_models)} Ollama models: {available_ollama_models}"
                )
                ...
                logger.warning(
                    f"Could not get Ollama models list: {e}, falling back to configured model"
                )
                # Fallback: just show configured model
                models.append(
                    {
                        "name": backend_name,
                        "display_name": backend_display_names.get(
                            backend_name, backend_name.title()
                        ),
                        "available": translator.is_ready,
                        "description": backend_descriptions.get(
                            backend_name, f"OpenAI-compatible: {backend_name}"
                        ),
                        "backend": "openai_compatible",
                        "endpoint": translator.config.base_url,
    ...
    [m for m in models if m.get("available", False)]
    recommended = (
        "llama"
        if any(m["name"] == "llama" and m["available"] for m in models)
        else ("nllb" if any(m["name"] == "nllb" and m["available"] for m in models) else "auto")
    )
    ...
    return jsonify(
        {
            ...
            "auto_selection_priority": [
                "llama",
                "nllb",
            ],  # Order of preference when "auto" is selected
        }
    )
    ...
    try:
        ...
        async def translate_to_language(target_lang: str):
            ...
            try:
                ...
                use_llama = (
                    (model == "llama" or (model == "auto" and not openai_backend))
                    and llama_translator
                    and llama_translator.is_ready
                )
                use_nllb = (
                    (model == "nllb" or (model == "auto" and not use_llama and not openai_backend))
                    and nllb_translator
                    and nllb_translator.is_ready
                )
...
async def api_translate_text():
    ...
    try:
        ...
        logger.info(
            f"API translate request: text={text[:50]}... target={target_language} model={model}"
        )
    ...
    try:
        ...
        def generate_stream():
            ...
            try:
                ...
                yield from loop.run_until_complete(stream_translation())
...
async def translate_with_continuity():
    ...
    try:
        ...
        logger.info(
            f"Continuity translation request: session={session_id}, chunk={chunk_id}, clean_text='{clean_text[:50]}...'"
        )

        # Process with continuity management (no deduplication needed)
        result = asyncio.run(
            translation_service.translate_with_continuity(
                text=clean_text,
                session_id=session_id,
                target_language=target_language,
                source_language=source_language,
...
async def process_clean_text():
    ...
    try:
        ...
        logger.info(
            f"Processing clean text from whisper service: session={session_id}, text='{clean_text[:50]}...'"
        )
        ...
        return jsonify({"status": "success", "translation_result": result, "metadata": metadata})
...
async def create_session():
    ...
    try:
        ...
        session_config = asyncio.run(
            translation_service.create_session(session_id, data.get("config"))
        )
            ...
            async def stream_to_client():
                try:
                    async for chunk in translation_service.translate_stream(translation_request):
                        socketio.emit(
                            "translation_chunk",
                            {"chunk": chunk, "session_id": translation_request.session_id},
                            room=request.sid,
                        )
                    ...
                    socketio.emit(
                        "translation_error",
                        {"error": str(e), "session_id": translation_request.session_id},
                        room=request.sid,
                    )
    ...
    try:
        ...
        if filter_active:
            ...
            source_lang, target_lang = (
                language_pair.split("-") if "-" in language_pair else ("auto", language_pair)
            )
...
def create_prompt():
    ...
    try:
        ...
        return jsonify({"message": "Prompt created successfully", "prompt_id": prompt.id}), 201
...
def update_prompt(prompt_id):
    ...
    try:
        ...
        return jsonify({"message": "Prompt updated successfully", "prompt_id": prompt_id})
...
def delete_prompt(prompt_id):
    ...
    try:
        ...
        if not success:
            return jsonify(
                {"error": "Prompt not found or cannot be deleted (default prompts are protected)"}
            ), 404

        return jsonify({"message": "Prompt deleted successfully", "prompt_id": prompt_id})
...
def get_prompt_categories():
    ...
    categories = [
        {
            ...
            "description": "Standard translation prompts",
        },
        {
    ...
    return jsonify({"categories": categories, "total_count": len(categories)})
...
async def translate_with_custom_prompt():
    ...
    try:
        ...
        if not built_prompt:
            return jsonify(
                {"error": "Failed to build prompt - check prompt template and variables"}
            ), 400
