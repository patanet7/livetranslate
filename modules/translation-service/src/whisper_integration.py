...
class WhisperTranslationBridge:
    ...
    def __init__(self, translation_service: TranslationService, whisper_url: str = "http://whisper-service:5001"):
        """Initialize the bridge"""
        self.translation_service = translation_service
        self.whisper_url = whisper_url
        self.whisper_ws_url = whisper_url.replace("http://", "ws://").replace("https://", "wss://") + "/ws"
    ...
    async def start_real_time_translation(self, session_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def process_transcription(self, session_id: str, transcription_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def stream_translations(self, session_id: str, transcription_stream: AsyncGenerator[Dict[str, Any], None]) -> AsyncGenerator[Dict[str, Any], None]:
        """
        ...
        try:
            async for transcription_data in transcription_stream:
                translation_result = await self.process_transcription(session_id, transcription_data)
    ...
    async def check_whisper_health(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.whisper_url}/api/health", timeout=aiohttp.ClientTimeout(total=5)) as response:
