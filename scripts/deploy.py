        ...
        if self.detected_hardware["os"] == "windows":
            try:
                result = subprocess.run(
                    ["driverquery", "/fo", "csv"], capture_output=True, text=True, timeout=10
                )
    ...
    def generate_compose_override(self) -> Path:
        ...
        if self.hardware_info["npu"]:
            override_config["services"]["whisper-npu-server"] = {
                "environment": ["OPENVINO_DEVICE=NPU", "NPU_OPTIMIZATION=true"],
                "deploy": {"resources": {"limits": {"memory": "6G", "cpus": "3.0"}}},
            }

        # GPU-specific overrides
        if self.hardware_info["gpu"]:
            override_config["services"]["whisper-gpu-server"] = {
                "deploy": {"resources": {"limits": {"memory": "10G", "cpus": "6.0"}}}
            }
            override_config["services"]["translation-server"] = {
                "deploy": {"resources": {"limits": {"memory": "12G", "cpus": "8.0"}}}
            }

        # CPU-only optimizations
        if not self.hardware_info["npu"] and not self.hardware_info["gpu"]:
            override_config["services"]["whisper-cpu-server"] = {
                "deploy": {"resources": {"limits": {"memory": "8G", "cpus": "6.0"}}}
            }
...
class DockerDeployer:
    ...
    def get_compose_command(self, action: str, extra_args: list[str] | None = None) -> list[str]:
    ...
    def deploy(self, services: list[str] | None = None, detached: bool = True) -> bool:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LiveTranslate Intelligent Deployment Script")
    parser.add_argument(
        "--action",
    ...
    try:
        ...
        env_gen.generate_compose_override()
