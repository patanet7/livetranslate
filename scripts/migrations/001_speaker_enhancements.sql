...
LEFT JOIN bot_sessions.speaker_identities si
    ON t.session_id = si.session_id AND t.speaker_id = si.speaker_label
LEFT JOIN bot_sessions.translations tr
    ON t.transcript_id = tr.source_transcript_id
WHERE t.speaker_id IS NOT NULL
GROUP BY t.session_id, t.speaker_id, si.identified_name, t.speaker_name,
