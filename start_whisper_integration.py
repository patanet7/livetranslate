...
class ServiceManager:
        ...
        while self.running:
            for name, process in list(self.services.items()):
                if process.poll() is not None:
                    logger.error(f"❌ {name} service has stopped unexpectedly (exit code: {process.returncode})")
                    self.running = False
                    break
            time.sleep(5)
    ...
    def run(self):
        ...
        required_paths = [
            ...
            Path(__file__).parent / "modules" / "orchestration-service" / "src" / "orchestration_service.py"
        ]
